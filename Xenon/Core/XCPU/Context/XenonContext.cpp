...
namespace Xe::XCPU {
  ...
  bool XenonContext::HandleSOCWrite(u64 writeAddr, const u8* data, size_t byteCount) {
    ...
    } else if (writeAddr >= XE_SOCSECOTP_BLOCK_START
               && writeAddr < XE_SOCSECOTP_BLOCK_START + XE_SOCSECOTP_BLOCK_SIZE) {
      // Secure OTP
      return HandleSecOTPWrite(writeAddr, data, byteCount);
    } else if (writeAddr >= XE_SOCSECRNG_BLOCK_START
               && writeAddr < XE_SOCSECRNG_BLOCK_START + XE_SOCSECRNG_BLOCK_SIZE) {
      // Secure RNG
  ...
  bool XenonContext::HandlePRVWrite(u64 writeAddr, const u8* data, size_t byteCount) {
    ...
    switch (writeAddr) {
      ...
      case 0x611A0ULL:
        ...
        // TimeBase Control
        LOG_WARNING(Xenon, "SoC PRV: TimeBase Control being set 0x{:X}, enabled: {}, divider: 0x{:X}.", dataIn,
                    socPRVBlock.get()->TimebaseControl.AsBITS.TimebaseEnable,
                    socPRVBlock.get()->TimebaseControl.AsBITS.TimebaseDivider);
        break;
      case 0x61188ULL:
        // CPU VID Register
