...
constexpr u32 MakeSwizzleMask(u32 x, u32 y, u32 z, u32 w) {
  return ((x & 0x3) << 0) | ((y & 0x3) << 2) | ((z & 0x3) << 4) | ((w & 0x3) << 6);
}
...
inline bool IsPackToHi(u32 flags) {
  return (flags & PACK_TYPE_TO_HI) == PACK_TYPE_TO_HI;
}
inline bool IsPackToLo(u32 flags) { return !IsPackToHi(flags); }
inline bool IsPackInUnsigned(u32 flags) {
  return (flags & PACK_TYPE_IN_UNSIGNED) == PACK_TYPE_IN_UNSIGNED;
}
inline bool IsPackOutUnsigned(u32 flags) {
  return (flags & PACK_TYPE_OUT_UNSIGNED) == PACK_TYPE_OUT_UNSIGNED;
}
inline bool IsPackOutSaturate(u32 flags) {
...
enum Opcode {
  ...
  OPCODE_BRANCH_CONDITIONAL_CTR,
  OPCODE_BRANCH_CONDITIONAL_LR,
  OPCODE_ASSIGN,
  OPCODE_CAST,
  OPCODE_ZERO_EXTEND,
  OPCODE_SIGN_EXTEND,
  ...
  OPCODE_BUILD_CR_SIGNED,
  OPCODE_BUILD_CR_UNSIGNED,
  OPCODE_SET_CR_FIELD,
  OPCODE_CALL_INTERPRETER,
  OPCODE_LOAD_RESERVED,
  OPCODE_STORE_RESERVED,
  OPCODE_STORAGE_EXCEPTION_CHECK,
  OPCODE_SYNC_EXCEPTION_CHECK,
  OPCODE_VD_SWAP,
  OPCODE_SYSCALL,
  OPCODE_RFID,
  OPCODE_MFSPR,
...
enum OpcodeFlags {
  ...
  OPCODE_FLAG_NEEDS_EXCEPTION_CHECK = (1 << 8),
};
