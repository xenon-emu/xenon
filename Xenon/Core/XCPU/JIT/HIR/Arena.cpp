...
void *Arena::Alloc(size_t size, size_t align) {
  ASSERT_MSG(
    align > 0 && std::has_single_bit(align) && align <= 16,
  ...
  if (active_chunk_) {
    if (active_chunk_->capacity - active_chunk_->offset <
      ...
      if (!next) {
        ASSERT_MSG(size + get_padding() < chunk_size_,
  ...
  u8 *p = active_chunk_->buffer + active_chunk_->offset;
  active_chunk_->offset += size;
  ASSERT_MSG((reinterpret_cast<size_t>(p) & (align - 1)) == 0,
    "alignment failed");
  return p;
}
...
Arena::Chunk::Chunk(size_t chunk_size)
  ...
  ASSERT_MSG((reinterpret_cast<size_t>(buffer) & size_t(15)) == 0,
