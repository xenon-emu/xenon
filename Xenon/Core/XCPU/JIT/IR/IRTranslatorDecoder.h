...
namespace Xe::XCPU::JIT {
  //=============================================================================
  // IR Translation Opcode Decoder
  //=============================================================================

  class IRTranslatorDecoder {
  public:
    IRTranslatorDecoder() { FillTables(); }
    ~IRTranslatorDecoder() = default;

    // Decode and return handler for instruction
    IRTranslatorHandler Decode(u32 instr) const noexcept {
      // NOP handling
      if (instr == 0x60000000) {
        return &IRTranslate_nop;
      }

      // Lookup in table
      IRTranslatorHandler handler = table[DecodeInstr(instr)];

      // If handle is valid, return it
      if (handler != &IRTranslate_invalid) {
        return handler;
      }

      // Not found, check VMX128
#define GET_HANDLER(name) &IRTranslate_##name
      // VMX128 Lookup.
...
#undef GET_HANDLER
    ...
    const std::array<IRTranslatorHandler, 0x20000> &GetTable() const noexcept {
      return table;
    }

  private:
    // Instruction info for table filling
    struct InstrInfo {
      u32 value;
      IRTranslatorHandler handler;
      IRTranslatorHandler handlerRc;
      u32 magnitude = 0;
    };

    u32 DecodeInstr(u32 instr) const {
      return ((instr >> 26) | (instr << 6)) & 0x1FFFF; // Rotate + mask
    }

    // Fast lookup table
    std::array<IRTranslatorHandler, 0x20000> table;

    void FillTables() {
#define GET_(name) &IRTranslate_##name
...
#undef GETRC
    ...
    void FillTable(u32 mainOp, u32 count, s32 sh, std::initializer_list<InstrInfo> entries) noexcept {
      for (const auto &v : entries) {
        if (sh < 11 && sh >= 0) {
          // Standard table expansion
          for (u32 i = 0; i < 1u << (v.magnitude + (11 - sh - count)); i++) {
            for (u32 j = 0; j < 1u << sh; j++) {
              const u32 k = (((i << (count - v.magnitude)) | v.value) << sh) | j;
              table[(k << 6) | mainOp] = (k & 1) ? v.handlerRc : v.handler;
            }
          }
        }
        else {
          // Special fallback for main opcodes (sh == -1)
          for (u32 i = 0; i < 1u << 11; i++) {
            table[(i << 6) | v.value] = (i & 1) ? v.handlerRc : v.handler;
          }
        }
      }
    }
  ...
  // Global translator decoder instance
  inline IRTranslatorDecoder irTranslatorDecoder;
