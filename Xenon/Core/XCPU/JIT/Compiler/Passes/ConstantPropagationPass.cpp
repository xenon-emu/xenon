    ...
    while (i) {
      ...
      case OPCODE_CONVERT:
        if (i->src1.value->IsConstant()) {
          TypeName targetType = v->type;
          v->set_from(i->src1.value);
          v->Convert(targetType, RoundMode(i->flags));
          i->Remove();
          result = true;
        }
        break;
      ...
      case OPCODE_MUL:
        if (i->src1.value->IsConstant() && i->src2.value->IsConstant()) {
          ...
          if (s2->type != VEC128_TYPE && s2->IsConstantOne()) {
            ...
            auto &c = s2->constant;
            if (c.v128.flt[0] == 1.f && c.v128.flt[1] == 1.f &&
              c.v128.flt[2] == 1.f && c.v128.flt[3] == 1.f) {
              i->Replace(&OPCODE_ASSIGN_info, 0);
              i->set_src1(s1);
              result = true;
            }
