 ...
 * License: GPL2
 */
template<typename T, u32 I, u32 N> using PPCBitfield = bitfield<T, sizeof(T) * 8 - N - I, N>;
...
union uFPSCR {
  ...
  uFPSCR() = default;
  explicit uFPSCR(u32 hexValue)
      : FPSCR_Hex{hexValue & fpscrMask} { }

  u8& operator[](u8 bitIdx) { return bits[bitIdx]; }

  uFPSCR& operator=(u32 inValue) {
    FPSCR_Hex = inValue & fpscrMask;
    return *this;
  }
...
struct PPUExceptionEvent {
  ...
  u16 priority = 0; // Lower value = higher priority.
  u16 progType = 0; // Program-exception subtype (FPU/ILL/PRIV/TRAP) when type == ppuProgramEx.
  u64 srr1Set = 0;  // Extra cause bits OR'd into (H)SRR1 at delivery.
};
...
struct sPPUThread {
  ...
  // Pending-exception event queue.
  // Kept sorted by ascending priority (highest-priority event at index 0).
  // The pendingExcMask is a fast derived summary of the queued types.
  PPUExceptionEvent pendingExc[kMaxPendingExc]{};
  u8 pendingExcCount = 0;
  u16 pendingExcMask = 0;

  // Enqueue exception (a single eExceptionBitmask bit). Idempotent per type.
  // For ppuProgramEx, progType selects the program subtype. Defined out-of-line below (needs the eExceptionBitmask
  // priority table).
  void RaiseExc(u16 type, u16 progType = 0);
...
inline void sPPUThread::RaiseExc(u16 type, u16 progType) {
  ...
  // Already queued?
  if (pendingExcMask & type) {
    if (type == ppuProgramEx) {
      for (u8 i = 0; i < pendingExcCount; ++i)
        if (pendingExc[i].type == ppuProgramEx) {
          pendingExc[i].progType = progType;
          break;
        }
    }
    return;
  }
  if (pendingExcCount >= kMaxPendingExc) return; // Queue full (cannot happen: one slot per architected type).
  PPUExceptionEvent ev{};
  ev.type = type;
  ev.priority = ppuExcPriority(type);
  ev.progType = progType;
  // Priority-ordered insertion (stable: equal priority keeps insertion order).
  u8 pos = pendingExcCount;
  while (pos > 0 && pendingExc[pos - 1].priority > ev.priority) {
    pendingExc[pos] = pendingExc[pos - 1];
    --pos;
  }
  pendingExc[pos] = ev;
  ++pendingExcCount;
...
// Remove type from the pending-exception queue and refresh the derived summary.
inline void sPPUThread::ClearExc(u16 type) {
  if (!(pendingExcMask & type)) return;
  u8 dst = 0;
  for (u8 i = 0; i < pendingExcCount; ++i)
    if (pendingExc[i].type != type) pendingExc[dst++] = pendingExc[i];
  pendingExcCount = dst;
  pendingExcMask &= ~type;
}
