...
void PPCInterpreter::PPCInterpreter_tw(PPU_STATE *ppuState) {
  ...
  s32 a = static_cast<s32>(GPR(rA));
  s32 b = static_cast<s32>(GPR(rB));
...
void PPCInterpreter::PPCInterpreter_mtspr(PPU_STATE *ppuState) {
  ...
  case SPR_RMOR:
    ...
    break;
  case SPR_HDEC:
    ppuState->SPR.HDEC = (u32)GPR(rD);
    break;
  case SPR_LPIDR:
    ppuState->SPR.LPIDR = (u32)GPR(rD);
    break;
  case SPR_SPRG0:
  ...
  case SPR_TBU_WO:
    ...
    break;
  default:
    LOG_ERROR(Xenon, "{}(Thrd{:#d}) SPR {:#x} ={:#x}", ppuState->ppuName, (u8)ppuState->currentThread, spr, GPR(rD));
    break;
  }
}
