...
void PPCInterpreter::ppcExecuteSingleInstruction(sPPEState* ppeState) {
  ...
  if (thread.CIA == 0x0200C870) { GPR(5) = 0; }
  ...
  if (thread.CIA == 0x0200C7F0) { GPR(3) = 0; }
  ...
  if (static_cast<u32>(thread.CIA) == 0x801AF580) { return; }
  ...
  if (static_cast<u32>(thread.CIA) == 0x800F8E20) { LOG_INFO(Xenon, "*** VdSwap ***"); }
  ...
  if (static_cast<u32>(thread.CIA) == 0x0) { LOG_DEBUG(Xenon, "Breakpoint HIT."); }

  // This is to set a PPU0[Thread0] breakpoint.
  if (thread.SPR.PIR == 0) { u8 a = 0; }
...
#define curThread thread
  ...
  switch (trapNumber) {
    ...
    case 0x18:
      ...
      break;
    default: LOG_WARNING(Xenon, "Unimplemented trap! trapNumber = '0x{:X}'", trapNumber); break;
  }
