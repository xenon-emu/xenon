...
template<typename T> struct addResult {
  ...
  addResult(T a, T b, bool sfBitMode)
      : result(a + b)
      , carry(sfBitMode ? (result < a) : (static_cast<u32>(result) < static_cast<u32>(a))) { }
  ...
  static constexpr addResult<T> addBits(T a, T b, bool sfBitMode) { return {a, b, sfBitMode}; }

  static constexpr addResult<T> addBits(T a, T b, bool c, bool sfBitMode) { return {a, b, c, sfBitMode}; }
};
...
inline void ppuSetXerOv(sPPEState* ppeState, bool inbit) {
  ...
  if (xer.CA) curThread.SPR.XER.CA = 1;
  if (xer.SO || inbit) curThread.SPR.XER.SO = 1;
...
void PPCInterpreter::PPCInterpreter_addx(sPPEState* ppeState) {
  ...
  */
  ...
  if (_instr.rc) { RECORD_CR0(GPRi(rd)); }
}

// Add + OE
void PPCInterpreter::PPCInterpreter_addox(sPPEState* ppeState) { PPCInterpreter_addx(ppeState); }
...
// Add Carrying + OE
void PPCInterpreter::PPCInterpreter_addcox(sPPEState* ppeState) { PPCInterpreter_addcx(ppeState); }
...
void PPCInterpreter::PPCInterpreter_addex(sPPEState* ppeState) {
  ...
  */
  ...
  // _rc
  if (_instr.rc) { RECORD_CR0(add.result); }
}

// Add Extended + OE
void PPCInterpreter::PPCInterpreter_addeox(sPPEState* ppeState) { PPCInterpreter_addex(ppeState); }
...
void PPCInterpreter::PPCInterpreter_addic(sPPEState* ppeState) {
  ...
  */
  ...
  // _rc
  if (_instr.main & 1) { RECORD_CR0(add.result); }
}
...
void PPCInterpreter::PPCInterpreter_addmex(sPPEState* ppeState) {
  ...
  */
  ...
  if (_instr.oe) {
    bool ovSet = false;
    if (curThread.SPR.MSR.SF) {
      ovSet = (static_cast<u64>(RA) >> 63 == 1) && (static_cast<u64>(RA) >> 63 != GPRi(rd) >> 63);
    } else {
      ovSet = (static_cast<u32>(RA) >> 31 == 1) && (static_cast<u32>(RA) >> 31 != static_cast<u32>(GPRi(rd)) >> 31);
    }
    ppuSetXerOv(ppeState, ovSet);
  }
...
void PPCInterpreter::PPCInterpreter_addmeox(sPPEState* ppeState) { PPCInterpreter_addmex(ppeState); }
  ...
  */
  ...
  if (_instr.oe) {
    ...
    if (curThread.SPR.MSR.SF) {
      ...
      ovSet = (static_cast<u32>(RA) >> 31 == 0) && (static_cast<u32>(RA) >> 31 != static_cast<u32>(GPRi(rd)) >> 31);
    }
    ppuSetXerOv(ppeState, ovSet);
  }
...
void PPCInterpreter::PPCInterpreter_addzeox(sPPEState* ppeState) { PPCInterpreter_addzex(ppeState); }
...
void PPCInterpreter::PPCInterpreter_andx(sPPEState* ppeState) {
  ...
  */
  ...
  // _rc
  if (_instr.rc) { RECORD_CR0(GPRi(ra)); }
}
...
void PPCInterpreter::PPCInterpreter_crand(sPPEState* ppeState) {
  ...
  if (crAnd & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_crandc(sPPEState* ppeState) {
  ...
  if (crAndc & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_creqv(sPPEState* ppeState) {
  ...
  if (crEqv & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_crnand(sPPEState* ppeState) {
  ...
  if (crNand & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_crnor(sPPEState* ppeState) {
  ...
  if (crNor & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_cror(sPPEState* ppeState) {
  ...
  if (crOr & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_crorc(sPPEState* ppeState) {
  ...
  if (crOrc & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_crxor(sPPEState* ppeState) {
  ...
  if (crXor & 1) BSET(curThread.CR.CR_Hex, 32, _instr.crbd);
  else BCLR(curThread.CR.CR_Hex, 32, _instr.crbd);
}
...
void PPCInterpreter::PPCInterpreter_divdox(sPPEState* ppeState) {
  ...
  */
  ...
  // _oe
  if (_instr.oe) { ppuSetXerOv(ppeState, RB == 0); }
...
void PPCInterpreter::PPCInterpreter_divwx(sPPEState* ppeState) {
  ...
  */
  ...
  if (_instr.oe) { ppuSetXerOv(ppeState, o); }
...
// Divide Word + OE
void PPCInterpreter::PPCInterpreter_divwox(sPPEState* ppeState) { PPCInterpreter_divwx(ppeState); }
...
void PPCInterpreter::PPCInterpreter_divwuox(sPPEState* ppeState) { PPCInterpreter_divwux(ppeState); }
...
void PPCInterpreter::PPCInterpreter_mftb(sPPEState* ppeState) {
  ...
  */
  ...
  switch (spr) {
    case TBLRO: GPRi(rd) = xenonContext->timeBase.ReadTB(); break;
    case TBURO: GPRi(rd) = (GPRi(rd) = xenonContext->timeBase.ReadTB() >> 32); break;

    default: LOG_CRITICAL(Xenon, "MFTB -> Illegal instruction form!"); break;
  }
}
...
void PPCInterpreter::PPCInterpreter_mullwx(sPPEState* ppeState) {
  ...
  */
  ...
  if (_instr.oe) { ppuSetXerOv(ppeState, s64(GPRi(rd)) < INT32_MIN || s64(GPRi(rd)) > INT32_MAX); }
...
void PPCInterpreter::PPCInterpreter_mullwox(sPPEState* ppeState) { PPCInterpreter_mullwx(ppeState); }
...
// Negate + OE
void PPCInterpreter::PPCInterpreter_negox(sPPEState* ppeState) { PPCInterpreter_negx(ppeState); }
...
void PPCInterpreter::PPCInterpreter_rlwinmx(sPPEState* ppeState) {
  ...
  */
  ...
  GPRi(ra)
    = duplicate32(rotl32(static_cast<u32>(GPRi(rs)), _instr.sh32)) & PPCRotateMask(32 + _instr.mb32, 32 + _instr.me32);
...
void PPCInterpreter::PPCInterpreter_srawix(sPPEState* ppeState) {
  ...
  */
  ...
  XER_SET_CA((RS < 0) && (static_cast<u32>(GPRi(ra) << _instr.sh32) != static_cast<u32>(RS)));
...
void PPCInterpreter::PPCInterpreter_subfcox(sPPEState* ppeState) { PPCInterpreter_subfcx(ppeState); }
...
// Subtract From + OE
void PPCInterpreter::PPCInterpreter_subfox(sPPEState* ppeState) { PPCInterpreter_subfx(ppeState); }
...
void PPCInterpreter::PPCInterpreter_subfeox(sPPEState* ppeState) { PPCInterpreter_subfex(ppeState); }
...
void PPCInterpreter::PPCInterpreter_subfmeox(sPPEState* ppeState) { PPCInterpreter_subfmex(ppeState); }
...
void PPCInterpreter::PPCInterpreter_subfzeox(sPPEState* ppeState) { PPCInterpreter_subfzex(ppeState); }
