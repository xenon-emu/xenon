...
u32 PPCInterpreter::CRCompS32(PPU_STATE *ppuState, u32 num1, u32 num2) {
  ...
  if (static_cast<sl32>(num1 < num2))
    BSET(CR, 4, CR_BIT_LT);
  else if (static_cast<sl32>(num1 > num2))
    BSET(CR, 4, CR_BIT_GT);
  else
    BSET(CR, 4, CR_BIT_EQ);
...
u32 PPCInterpreter::CRCompS64(PPU_STATE *ppuState, u64 num1, u64 num2) {
  ...
  if (static_cast<s64>(num1 < num2))
    BSET(CR, 4, CR_BIT_LT);
  else if (static_cast<s64>(num1 > num2))
    BSET(CR, 4, CR_BIT_GT);
  else
    BSET(CR, 4, CR_BIT_EQ);
...
void PPCInterpreter::ppcDebugLoadImageSymbols(PPU_STATE *ppuState,
  ...
  Base::Log::NoFmtMessage(Base::Log::Class::DebugPrint, Base::Log::Level::Guest, fmt::format("Loaded: {} at address {:#x} - {:#x}\n", moduleName, Kdinfo.BaseOfDll, (Kdinfo.BaseOfDll + Kdinfo.SizeOfImage)));
}
