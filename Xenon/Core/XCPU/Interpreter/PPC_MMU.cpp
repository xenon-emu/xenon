...
void PPCInterpreter::MMUWrite(XENON_CONTEXT *cpuContext, PPU_STATE *ppuState,
  ...
  if (socWrite && EA == POST_BUS_ADDR) {
    Xe::XCPU::POSTBUS::POST(byteswap_le<u64>(data));
    return;
  }
...
u8 PPCInterpreter::MMURead8(PPU_STATE *ppuState, u64 EA, bool speculativeLoad) {
  ...
  *reinterpret_cast<decltype(data)*>((data_ptr + sizeof(u64)) - sizeof(u8)) = data;
  return static_cast<u8>(*data_ptr);
#else
  u8 data = 0;
  data = MMURead(intXCPUContext, ppuState, EA, sizeof(data), speculativeLoad);
...
u16 PPCInterpreter::MMURead16(PPU_STATE *ppuState, u64 EA, bool speculativeLoad) {
  ...
  data = MMURead(intXCPUContext, ppuState, EA, sizeof(data), speculativeLoad);
  u64* data_ptr = reinterpret_cast<u64*>(&data);
  *reinterpret_cast<decltype(data)*>((data_ptr + sizeof(u64)) - sizeof(u16)) = data;
  return static_cast<u16>(*data_ptr);
#else
  u16 data = 0;
  data = MMURead(intXCPUContext, ppuState, EA, sizeof(data), speculativeLoad);
...
u32 PPCInterpreter::MMURead32(PPU_STATE *ppuState, u64 EA, bool speculativeLoad) {
  ...
  data = MMURead(intXCPUContext, ppuState, EA, sizeof(data), speculativeLoad);
  u64* data_ptr = reinterpret_cast<u64*>(&data);
  *reinterpret_cast<decltype(data)*>((data_ptr + sizeof(u64)) - sizeof(u32)) = data;
  return static_cast<u32>(*data_ptr);
#else
  u32 data = 0;
  data = MMURead(intXCPUContext, ppuState, EA, sizeof(data), speculativeLoad);
