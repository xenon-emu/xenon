...
//
	...
	typedef union _SOCINTS_IPIGR {
		...
		struct {
			...
			u64 VectorNumber : 5;
			u64 Reserved2 : 9;
			u64 DestinationId : 6; // Which cores is this IPI taegeted to, 1 << corenum/LogicalIdentification.
			u64 Reserved3 : 42;
		} AsBITS;
#else
		struct {
	...
	typedef struct _SOCINTS_PROCESSOR_BLOCK {
		...
		SOCINTS_IPIGR IpiGeneration;										// 16 0x10
		u64 Reserved1;																	// 24 0x18
		u64 InterruptCapture;														// 32 0x20	The IRR specifies which interrupts are pending acknowledgement.
		u64 InterruptAssertion;													// 40 0x28
		u64 InterruptInService;													// 48 0x30  The ISR register specifies which interrupts have been acknowledged, 
																										//					but are still waiting for an End Of Interrupt (EOI).
		u64 InterruptTriggerMode;												// 56 0x38
		u64 Reserved2[2];																// 64
		SOCINTS_VECTOR InterruptAcknowledge;						// 80 0x50
		SOCINTS_VECTOR InterruptAcknowledgeAutoUpdate;	// 88 0x58
		u64 EndOfInterrupt;															// 96 0x60
		SOCINTS_VECTOR EndOfInterruptAutoUpdate;				// 104 0x68
		SOCINTS_VECTOR SpuriousVector;									// 112 0x70
		u64 Reserved3[15];															// 120
		u64 ThreadReset;																// 240 0xF0
		u64 Reserved4[481];															// 248
	} SOCINTS_PROCESSOR_BLOCK, * PSOCINTS_PROCESSOR_BLOCK; // 4096b
	...
	} SOCINTS_BLOCK, * PSOCINTS_BLOCK; // 32768/0x8000
}