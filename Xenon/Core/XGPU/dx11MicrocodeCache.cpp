...
CDX11MicrocodeCache::TMicrocodeHash CDX11MicrocodeCache::ComputeHash( const void* shaderCode, const u32 shaderCodeSize )
{
	return XenonGPUCalcCRC64( shaderCode, shaderCodeSize );
}

CDX11MicrocodeShader* CDX11MicrocodeCache::GetCachedPixelShader( const void* shaderCode, const u32 shaderCodeSize )
{
	// invalid data
	...
	if ( !newShader )
	{
		printf( "D3D: Failed to extract pixel shader from microcode hash 0x%016llX. Will not retry.", hash );
	...
	if ( !m_dumpPath.empty() )
	{
		...
		// dump original code
		{
			...
			if ( f )
			{
				...
				TextFileWriter tfw(f);
				decompiler.DisassembleShader( tfw, (const u32*) shaderCode, shaderCodeSize/4 );
...
CDX11MicrocodeShader* CDX11MicrocodeCache::GetCachedVertexShader( const void* shaderCode, const u32 shaderCodeSize )
{
	// invalid data
	if ( !shaderCode || !shaderCodeSize )
		return nullptr;
	...
	if ( !newShader )
	{
		printf( "D3D: Failed to extract vertex shader from microcode hash 0x%016llX. Will not retry.", hash );
