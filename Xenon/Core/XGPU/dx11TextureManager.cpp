...
u32 CDX11AbstractSurface::GetWidth() const
{
	return m_width;
}

u32 CDX11AbstractSurface::GetHeight() const
{
	return m_height;
}

u32 CDX11AbstractSurface::GetDepth() const
{
	return m_depth;
}

u32 CDX11AbstractSurface::GetRowPitch() const
{
	return m_sourceRowPitch;
}

u32 CDX11AbstractSurface::GetSlicePitch() const
{
	return m_sourceSlicePitch;
}

u32 CDX11AbstractSurface::GetSourceMemoryAddress() const
{
...
namespace Helper
{
	static void ConvertGPUEndianess(void* dest, const void* src, const u32 length, XenonGPUEndianFormat endianess)
	{
		// endianess
...
void CDX11AbstractSurface::Upload(const void* srcTextureData, void* destData, u32 destRowPitch, u32 destSlicePitch) const
{
	u8 TempData[8192];
	...
	for (u32 z = 0; z<m_depth; ++z)
	{
		...
		const u8* srcData = (const u8*)((u64)srcTextureData + m_sourceMemoryOffset + m_sourceSlicePitch*z);
		u8* targetData = (u8*)destData + (z*destSlicePitch);
		...
		if (m_sourceIsTiled)
		{
			...
			const u32 blockSize = m_sourceBlockSize;

			// Log of BPP (1, 2, 3)
			const u32 bppLog = (m_sourceBlockSize >> 2) + ((m_sourceBlockSize >> 1) >> (m_sourceBlockSize >> 2));

			// copy range
			const u32 copyWidth = m_sourceBlockWidth;
			const u32 copyHeight = std::min(m_sourceBlockHeight, m_height);
			...
			for (u32 y = 0, outputOffset = 0; y<copyHeight; ++y, destOffsetY += destRowPitch)
			{
				const u32 srcOffsetY = XenonTextureInfo::TiledOffset2DOuter(m_sourcePackedTileOffsetY + y, m_sourceWidth / m_sourceFormatBlockWidth, bppLog);

				u32 destOffsetX = destOffsetY;

				for (u32 x = 0; x<copyWidth; ++x, destOffsetX += blockSize)
				{
					const u32 srcX = m_sourcePackedTileOffsetX + x;
					const u32 srcY = m_sourcePackedTileOffsetY + y;
					const u32 srcOffset = XenonTextureInfo::TiledOffset2DInner(srcX, srcY, bppLog, srcOffsetY) >> bppLog;
		...
		{
			const u32 totalSize = destRowPitch * m_sourceHeight;
		...
		{
			const u32 copyPitch = std::min(m_sourceRowPitch, destRowPitch);
			u32 copyHeight = std::min(m_sourceBlockHeight, m_height);
			u32 copyWidth = m_width;
			for (u32 y = 0; y<copyHeight; ++y)
			{
				...
				{
					const u16* tempPtr = (const u16*)&TempData;
					u8* destWrite = (u8*)targetData;
					for (u32 x = 0; x<copyWidth; ++x, ++tempPtr, destWrite += 4)
					{
		...
		else
		{
			...
			const u32 copyPitch = std::min(m_sourceRowPitch, destRowPitch);
			u32 copyHeight = std::min(m_sourceBlockHeight, m_sourceHeight);
			for (u32 y = 0; y<copyHeight; ++y)
			{
...
CDX11AbstractTexture::CDX11AbstractTexture(RAM* ramPtr)
	: ram(ramPtr)
{
}

u32 CDX11AbstractTexture::GetBaseAddress() const
{
...
u32 CDX11AbstractTexture::GetBaseWidth() const
{
...
u32 CDX11AbstractTexture::GetBaseHeight() const
{
	if (m_sourceType == XenonTextureType::Texture_2D)
...
u32 CDX11AbstractTexture::GetBaseDepth() const
{
	if (m_sourceType == XenonTextureType::Texture_2D)
		return 1;
...
u32 CDX11AbstractTexture::GetNumMipLevels() const
{
	return m_sourceMips;
}

u32 CDX11AbstractTexture::GetNumArraySlices() const
{
	return m_sourceArraySlices;
}

IXenonGPUAbstractSurface* CDX11AbstractTexture::GetSurface(const u32 slice, const u32 mip)
{
...
bool CDX11AbstractTexture::CreateResources(ID3D11Device* device)
{
	...
	if (m_sourceType == XenonTextureType::Texture_2D)
	{
		...
		if (FAILED(hRet) || !tex)
		{
			printf("D3D: Error creating 2D texture: 0x%08X", hRet);
			return false;
		}
		...
		{
			tex->Release();
			printf("D3D: Error creating view for 2D texture: 0x%08X", hRet);
			return false;
		}
...
bool CDX11AbstractTexture::CreateSurfaces(ID3D11Device* device)
{
	...
	if (m_sourceType == XenonTextureType::Texture_2D)
	{
		...
		for (u32 slice = 0; slice<m_sourceArraySlices; ++slice)
		{
			for (u32 mip = 0; mip<m_sourceMips; ++mip)
			{
				const u32 memoryOffset = 0; // TODO
				...
				surf->m_width = std::max<u32>(1, m_sourceInfo.m_size2D.m_logicalWidth >> mip);
				surf->m_height = std::max<u32>(1, m_sourceInfo.m_size2D.m_logicalHeight >> mip);
				surf->m_depth = 1;

				surf->m_sourceWidth = std::max<u32>(1, m_sourceInfo.m_size2D.m_actualWidth >> mip);
				surf->m_sourceHeight = std::max<u32>(1, m_sourceInfo.m_size2D.m_actualHeight >> mip);
				surf->m_sourceBlockWidth = std::max<u32>(1, m_sourceInfo.m_size2D.m_actualBlockWidth >> mip);
				surf->m_sourceBlockHeight = std::max<u32>(1, m_sourceInfo.m_size2D.m_actualBlockHeight >> mip);
				...
				const u32 blockSize = m_sourceInfo.m_format->GetBlockSizeInBytes();
	...
	else if (m_sourceType == XenonTextureType::Texture_Cube)
	{
		...
		for (u32 slice = 0; slice < m_sourceInfo.m_depth; ++slice)
		{
			for (u32 mip = 0; mip < m_sourceMips; ++mip)
			{
				const u32 memoryOffset = 0; // TODO
				...
				surf->m_sourceFormat = m_sourceInfo.m_format->m_format;
				surf->m_sourceFormatBlockWidth = m_sourceInfo.m_format->m_blockWidth;
				surf->m_width = std::max<u32>(1, m_sourceInfo.m_sizeCube.m_logicalWidth >> mip);
				surf->m_height = std::max<u32>(1, m_sourceInfo.m_sizeCube.m_logicalHeight >> mip);
				surf->m_depth = 1;

				surf->m_sourceWidth = std::max<u32>(1, m_sourceInfo.m_sizeCube.m_actualWidth >> mip);
				surf->m_sourceHeight = std::max<u32>(1, m_sourceInfo.m_sizeCube.m_actualHeight >> mip);
				surf->m_sourceBlockWidth = std::max<u32>(1, m_sourceInfo.m_sizeCube.m_actualBlockWidth >> mip);
				surf->m_sourceBlockHeight = std::max<u32>(1, m_sourceInfo.m_sizeCube.m_actualBlockHeight >> mip);
...
bool CDX11AbstractTexture::CreateStagingBuffers(CDX11StagingTextureCache* stagingDataCache)
{
	for (u32 mip = 0; mip<m_sourceMips; ++mip)
	{
		u32 width = 1, height = 1, depth = 1, dims = 1;

		if (m_sourceType == XenonTextureType::Texture_1D)
		{
			width = std::max<u32>(1, m_sourceInfo.m_size1D.m_width >> mip);
			dims = 1;
		}
		else if (m_sourceType == XenonTextureType::Texture_2D || m_sourceType == XenonTextureType::Texture_Cube)
		{
			width = std::max<u32>(1, m_sourceInfo.m_size2D.m_logicalWidth >> mip);
			height = std::max<u32>(1, m_sourceInfo.m_size2D.m_logicalHeight >> mip);
			dims = 2;
		}
		else if (m_sourceType == XenonTextureType::Texture_3D)
		{
...
CDX11AbstractTexture* CDX11AbstractTexture::Create(ID3D11Device* device, CDX11StagingTextureCache* stagingCache, const struct XenonTextureInfo* textureInfo)
{
	...
	{
		printf("D3D: Unable to create wrapper for 2D texture of format %hs, width=%d, height=%d",
			XenonGPUTextureFormatName(format->m_format), textureInfo->m_width, textureInfo->m_height);
		return nullptr;
	}

	// Create texture wrapper
	std::unique_ptr< CDX11AbstractTexture > ret(new CDX11AbstractTexture());
...
void CDX11AbstractTexture::EnsureUpToDate(RAM* ram)
{
...
void CDX11AbstractTexture::Update(RAM* ramPtr)
{
	// get address for the SOURCE texture data
	const u32 baseAddress = GetBaseAddress();
	const void* srcData = (const void*)ramPtr->GetPointerToAddress(baseAddress);
	...
	for (u32 slice = 0; slice<m_sourceArraySlices; ++slice)
	{
		...
		{
			// target mip
			const u32 runtimeSliceIndex = slice*m_sourceMips + mip;
			...
			u32 destRowPitch = 0, destSlicePitch = 0;
...
CDX11AbstractTexture* CDX11TextureManager::FindTexture(const u32 baseAddress)
{
	TScopeLock lock(m_lock);
...
CDX11AbstractTexture* CDX11TextureManager::GetTexture(const u32 baseAddress, const u32 width, const u32 height, const XenonTextureFormat format)
{
	TScopeLock lock(m_lock);
	...
	info.m_format = XenonTextureFormatInfo::Get((u32)format);
	...
	if (!tex)
	{
		printf("D3D: Unable to create RT texture %dx%d, format %hs at 0x%08X", width, height, XenonGPUTextureFormatName(format), baseAddress);
		return nullptr;
	}

	// created
	printf("D3D: Created RT texture %dx%d, format %hs at 0x%08X", width, height, XenonGPUTextureFormatName(format), baseAddress);
...
CDX11AbstractTexture* CDX11TextureManager::GetTexture(const XenonTextureInfo* textureInfo)
{
	...
	if (!tex)
	{
		printf("D3D: Unable to upload texture %dx%d, format %hs at 0x%08X", textureInfo->m_width, textureInfo->m_height, XenonGPUTextureFormatName(textureInfo->m_format->m_format), textureInfo->m_address);
		return nullptr;
	}

	// created
	printf("D3D: Created texture %dx%d, format %hs at 0x%08X", textureInfo->m_width, textureInfo->m_height, XenonGPUTextureFormatName(textureInfo->m_format->m_format), textureInfo->m_address);
