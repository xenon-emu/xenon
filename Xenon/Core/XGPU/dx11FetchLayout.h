...
public:
	...
	static const u32 MAX_ELEMS = 16;
	static const u32 MAX_STREAMS = 16;

	// single element in stream
	struct ElementDesc
	{
		EFormat		m_format;		// data format
		u16		m_offset;		// offset from the stream start, in dwords
	};

	// stream description
	struct StreamDesc
	{
		u16		m_slot;					// original fetch slot
		u16		m_stride;				// stride (in dword, arbitraty)

		u8		m_numElems;				// number of elements in stream
...
public:
	...
	inline const u32 GetNumStreams() const { return m_numStreams; }

	// Get n-th stream
	inline const StreamDesc& GetStream(const u32 index) const { DEBUG_CHECK(index < m_numStreams); return m_streams[index]; }
