...
namespace DX11Microcode
{
	...
	public:
		...
		// block extraction
		inline const u32 GetNumCreatedBlocks() const { return (u32) m_createdBlocks.size(); }
		inline Block* GetCreatedBlock( const u32 index ) const { return m_createdBlocks[index]; }
	...
	protected:
		...
		virtual CodeExpr EmitVertexFetch( CodeExpr src, int fetchSlot, int fetchOffset, u32 stride, CXenonGPUMicrocodeTransformer::EFetchFormat format, bool isFloat, bool isSigned, bool isNormalized ) override final;
		...
		virtual void EmitExec( const u32 codeAddr, CXenonGPUMicrocodeTransformer::EFlowBlock type, CodeStatement preamble, CodeStatement code, CodeExpr condition, const bool endOfShader ) override final;
		virtual void EmitJump( const u32 targetAddr, CodeStatement preamble, CodeExpr condition ) override final;
		virtual void EmitCall( const u32 targetAddr, CodeStatement preamble, CodeExpr condition ) override final;

		virtual void EmitExportAllocPosition() override final;
		virtual void EmitExportAllocParam( const u32 size ) override final;
		virtual void EmitExportAllocMemExport( const u32 size ) override final;

	private:
		std::vector< Block* >	m_createdBlocks;

		bool					m_positionExported;
		u32					m_numParamExports;
		u32					m_numMemoryExports;
