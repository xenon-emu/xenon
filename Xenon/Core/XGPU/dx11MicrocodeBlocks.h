...
namespace DX11Microcode
{
	...
	public:
		...
		Block( u32 address, Statement* preamble, Statement* code, ExprNode* condition );
		Block( ExprNode* condition, u32 targetAddress, EBlockType type );
		~Block();

		inline const EBlockType GetType() const { return m_type; }

		inline const u32	GetAddress() const { return m_address; }
		inline const u32	GetTargetAddress() const { return m_targetAddrses; }
	...
	private:
		...
		u32			m_address;			// generalized address (used to link blocks together)
		u32			m_targetAddrses;	// target address - only for JUMP and CALL
	...
	public:
		...
		// random block access
		inline const u32 GetNumBlocks() const { return (u32) m_blocks.size(); }
		inline const Block* GetBlock( const u32 index ) const { return m_blocks[ index ]; }

		// compile
		static ControlFlowGraph* DecompileMicroCode( const void* code, const u32 codeLength, XenonShaderType shaderType );
	...
	public:
		...
		inline const u32 GetNumVertexFetches() const { return (u32) m_vertexFetches.size(); }
		inline const ExprVertexFetch* GetVertexFetch( const u32 index ) const { return m_vertexFetches[ index ]; }

		// get number of register export instructions (shader outputs)
		inline const u32 GetNumExportInstructions() const { return (u32) m_exports.size(); }
		inline const ExprWriteExportReg* GetExportInstruction( const u32 index ) const { return m_exports[ index ]; }

		// get number of used registers
		inline const u32 GetNumUsedRegisters() const { return (u32) m_usedRegisters.size(); }
		inline const u32 GetUsedRegister( const u32 index ) const { return m_usedRegisters[index]; }

		// get number of used interpolators
		inline const u32 GetNumUsedInterpolators() const { return m_numUsedInterpolators; }

		/// Get mask for used texture fetch slots
		inline const u32 GetTextureFetchSlotMask() const { return m_textureFetchSlotMask; }

		// get number of texture references
		inline const u32 GetNumUsedTextures() const { return (u32) m_usedTextures.size(); }
		inline const TextureRef& GetUsedTexture( const u32 index ) const { return m_usedTextures[ index ]; }

		// compile
		static Shader* DecompileMicroCode( const void* code, const u32 codeLength, XenonShaderType shaderType );
