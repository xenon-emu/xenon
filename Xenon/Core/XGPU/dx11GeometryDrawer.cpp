...
bool CDX11GeometryDrawer::ShaderData::SetData( CDX11MicrocodeCache* cache, const void* data, const u32 numWords )
{
	// swap data
	u32 words[ 8192 ];
	for ( u32 i=0; i<numWords; ++i )
	{
		words[i] = _byteswap_ulong( ((const u32*)data)[i] );
	}

	// decompile and cache the microcode
	if ( m_type == XenonShaderType::ShaderPixel )
	{
		CDX11MicrocodeShader* microcode = cache->GetCachedPixelShader( words, numWords * sizeof(u32) );
		m_changed = (m_microcode != microcode);
		m_microcode = microcode;
	}
	else if ( m_type == XenonShaderType::ShaderVertex )
	{
		CDX11MicrocodeShader* microcode = cache->GetCachedVertexShader( words, numWords * sizeof(u32) );
		m_changed = (m_microcode != microcode);
		m_microcode = microcode;
	}
...
CDX11GeometryDrawer::CDX11GeometryDrawer( ID3D11Device* dev, ID3D11DeviceContext* context, RAM *ramPtr )
	: m_device( dev )
...
void CDX11GeometryDrawer::SetPhysicalSize( const u32 width, const u32 height )
{
	if ( width && height )
	{
...
void CDX11GeometryDrawer::SetPixelShaderdCode( const void* data, const u32 numWords )
{
...
void CDX11GeometryDrawer::SetVertexShaderdCode( const void* data, const u32 numWords )
{
...
const u32 CDX11GeometryDrawer::GetActiveTextureFetchSlotMask() const
{
	return m_shaderTextureFetchSlots;
}

void CDX11GeometryDrawer::SetTexture( const u32 fetchSlot, CDX11AbstractTexture* runtimeTexture )
{
	DEBUG_CHECK( fetchSlot < MAX_TEXTURE_FETCH_SLOTS );
	m_textures[ fetchSlot ] = runtimeTexture;
}

void CDX11GeometryDrawer::SetSampler(const u32 fetchSlot, const XenonSamplerInfo& samplerInfo)
{
...
void CDX11GeometryDrawer::CreateDefaultTextures()
{
	// 1D texture
	{
		...
		// texture memory
		u8* data = new u8[size * 4];
		u8* writePtr = data;
		for (u32 x = 0; x < size; ++x, writePtr += 4)
		{
			writePtr[0] = x;
			writePtr[1] = 255 - x;
			writePtr[2] = (x & 7) << 4;
	...
	// 2D cube texture
	{
		...
		// texture memory
		u8* data = new u8[6 * size*size * 4];
		u8* writePtr = data;
		for (u32 i = 0; i < 6; ++i)
		{
			const u8 rMask = (i == 0) ? 0 : (i == 1) ? 255 : 127;
			const u8 gMask = (i == 2) ? 0 : (i == 3) ? 255 : 127;
			const u8 bMask = (i == 4) ? 0 : (i == 5) ? 255 : 127;
			for (u32 y = 0; y < size; ++y)
			{
				for (u32 x = 0; x < size; ++x, writePtr += 4)
				{
	...
	// 3D texture
	{
		...
		// texture memory
		u8* data = new u8[size*size*size * 4];
		u8* writePtr = data;
...
bool CDX11GeometryDrawer::RealizeVertexBuffers(  const class CXenonGPURegisters& regs, class IXenonGPUDumpWriter* traceDump, const class CDX11FetchLayout* layout, RAM *ram)
{
	...
	const u32 numStreams = layout->GetNumStreams();
	for ( u32 i=0; i<numStreams; ++i )
	{
		...
		const u32 fetchRegBase = (u32) XenonGPURegister::REG_SHADER_CONSTANT_FETCH_00_0 + (stream.m_slot * 2);
		...
		const void* sourceMemory = (const void*) ram->GetPointerToAddress(fetchEntry->address << 2); // address aligned to 4
		const u32 sourceSize = fetchEntry->size << 2; // size in dwords

		// dump the vertex streams
		if ( traceDump )
		{
			auto memoryAddr = ( fetchEntry->address << 2 ); // address aligned to 4
			traceDump->MemoryAccessRead( reinterpret_cast<u64>(ram->GetPointerToAddress(memoryAddr)), sourceSize, "VertexBuffer" );
		}
...
static u16 TempIndices[ 1024*1024 ];
...
{	
	...
	if ( primitiveType == XenonPrimitiveType::PrimitiveQuadList )
	{
		...
		const u32 quadCount = ds.m_indexCount/4;

		// create or convert index buffer
		if ( ds.m_indexData == nullptr )
		{
			// convert quad indices to triangle list indices
			u16* writePtr = TempIndices;
			for ( u32 i=0; i<quadCount; ++i, writePtr += 6 )
			{
				const u16 baseIndex = i*4;
		...
		else
		{
			// convert quad indices to triangle list indices
			u16* writePtr = TempIndices;
			const u16* readPtr = (const u16*) ds.m_indexData;
			for ( u32 i=0; i<quadCount; ++i, readPtr += 4, writePtr += 6 )
			{
...
bool CDX11GeometryDrawer::Draw( const class CXenonGPURegisters& regs, class IXenonGPUDumpWriter* traceDump, const struct CXenonGPUState::DrawIndexState& ds , RAM* ram)
{
	if ( !RealizeShaders() )
		return false;
	...
	if ( traceDump )
	{
		...
		traceDump->MemoryAccessRead( (u64) psCode.c_str(), (u32)psCode.length(), "PSSourceCode" );

		const auto& vsCode = vs->GetSourceCode();
		traceDump->MemoryAccessRead( (u64) vsCode.c_str(), (u32)vsCode.length(), "VSSourceCode" );
	}
	...
	if ( !RealizeVertexBuffers( regs, traceDump, fetchLayout,ram) )
		return false;
	...
	const u32 vertexCount = dsCopy.m_indexCount;
