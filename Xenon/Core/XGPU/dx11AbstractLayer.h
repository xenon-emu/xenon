...
public:
	...
	virtual bool SetDisplayMode( const u32 width, const u32 height ) override final;
	...
	// RT&DS interface
	virtual void BindColorRenderTarget( const u32 index, const XenonColorRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch ) override final;
	virtual void BindDepthStencil( const XenonDepthRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch ) override final;	
	virtual void UnbindColorRenderTarget( const u32 index ) override final;
	virtual void UnbindDepthStencil() override final;
	virtual void SetColorRenderTargetWriteMask( const u32 index, const bool enableRed, const bool enableGreen, const bool enableBlue, const bool enableAlpha ) override final;
	virtual void ClearColorRenderTarget( const u32 index, const float* clearColor, const bool flushToEDRAM ) override final;
	virtual void ClearDepthStencilRenderTarget( const float depthClear, const u32 stencilClear, const bool flushToEDRAM ) override final;
	virtual bool ResolveColorRenderTarget( const u32 srcIndex, const XenonColorRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, const u32 destBase, const u32 destLogicalWidth, const u32 destLogicalHeight, const u32 destBlockWidth, const u32 destBlockHeight, const XenonTextureFormat destFormat, const XenonRect2D& destRect ) override final;
	...
	virtual bool RealizeSurfaceSetup( u32& outMainWidth,  u32& outMainHeight ) override final;
	...
	virtual void EnableScisor( const u32 x, const u32 y, const u32 w, const u32 h ) override final;
	...
	virtual void SetStencilWriteMask( const u8 mask ) override final;
	virtual void SetStencilReadMask( const u8 mask ) override final;
	virtual void SetStencilRef( const u8 mask ) override final;
	...
	// Blending
	virtual void SetBlend( const u32 rtIndex, const bool isEnabled ) override final;
	virtual void SetBlendOp( const u32 rtIndex, const XenonBlendOp colorOp, const XenonBlendOp alphaOp ) override final;
	virtual void SetBlendArg( const u32 rtIndex, const XenonBlendArg colorSrc, const XenonBlendArg colorDest, const XenonBlendArg alphaSrc, const XenonBlendArg alphaDest ) override final;
	...
	virtual void SetPrimitiveRestartIndex( const u32 index ) override final;
	virtual bool RealizeRasterState() override final;
	
	// Shader interface
	virtual void SetPixelShader( const void* microcode, const u32 numWords ) override final;
	virtual void SetVertexShader( const void* microcode, const u32 numWords ) override final;
	virtual void SetPixelShaderConsts( const u32 firstVector, const u32 numVectors, const float* values ) override final;
	virtual void SetVertexShaderConsts( const u32 firstVector, const u32 numVectors, const float* values ) override final;
	virtual void SetBooleanConstants( const u32* boolConstants ) override final;
	virtual bool RealizeShaderConstants() override final;

	// Geometry interface
	virtual bool DrawGeometry( const class CXenonGPURegisters& regs, IXenonGPUDumpWriter* traceDump, const CXenonGPUState::DrawIndexState& ds, RAM*ram) override final;

	// Textures
	virtual u32 GetActiveTextureFetchSlotMask() const override final;
	virtual void SetTexture( const u32 fetchSlot, const XenonTextureInfo* texture ) override final;
	virtual void SetSampler(const u32 fetchSlot, const XenonSamplerInfo* sampler) override final;
...
private:
	...
	u32						m_depthStencilStateStencilRef;
	...
	// handle hacky clear
	bool HACK_ClearFromDraw(const CXenonGPURegisters& regs, IXenonGPUDumpWriter* traceDump, const CXenonGPUState::DrawIndexState& ds, RAM* ram);
};