...
bool CDX11ComputeShader::Load( ID3D11Device* dev, const void* data, const u32 dataSize )
{
	if ( !m_shader )
	{
		...
		{
			printf( "D3D: Failed to load compute shader '%hs'", m_name );
			return false;
		}
	}
...
bool CDX11GeometryShader::Load(ID3D11Device* dev, const void* data, const u32 dataSize)
{
	if (!m_shader)
	{
...
CDX11ConstantBuffer::CDX11ConstantBuffer( void* dataPtr, const u32 dataSize )
	: m_dataPtr( dataPtr )
	, m_dataSize( dataSize )
	, m_buffer( nullptr )
	, m_modified( false )
...
bool CDX11ConstantBuffer::Create( ID3D11Device* dev )
{
	if ( !m_buffer )
	{
		...
		{
			printf( "D3D: Failed to create constant buffer. Error = 0x%08X", hRet );
			return false;
		}
	}
...
bool CDX11ConstantBuffer::FlushChanges( ID3D11DeviceContext* context )
{
	...
	if ( m_modified )
	{
		...
		if ( FAILED( hRet ) )
		{
			printf( "D3D: Failed to map constant buffer memory. Error = 0x%08X", hRet );
			return false;
		}
...
CDX11BufferRing::CDX11BufferRing( ID3D11Device* dev, ID3D11DeviceContext* context, const u32 size )
	: m_device( dev )
	, m_context( context )
	, m_pos( 0 )
	, m_size( size )
...
void* CDX11BufferRing::Acquire( u32 size, u32 alignment, AllocInfo& outAlloc )
{
	DWORD mapFlags = 0;
	...
	// allocate offset
	D3D11_MAP mapType;
	const u32 alignedPos = (m_pos + (alignment-1)) & ~(alignment-1);
	...
	if ( FAILED(hRet) )
	{
		printf( "D3D: Map error in circular buffer: size=%d, error=0x%08X", size, hRet );
		return nullptr;
	}
	...
	return (u8*)mapData.pData + outAlloc.m_offset;
}
