...
void BlockTranslator::EmitExportAllocParam(const u32 size)
{
	m_numParamExports += size;
}

void BlockTranslator::EmitExportAllocMemExport(const u32 size)
{
...
CodeExpr BlockTranslator::EmitWriteReg(bool pixelShader, bool exported, int regIndex)
{
	if (exported)
	{
		...
		else
		{
			...
			{
				printf("Microcode: Vertex shader register %d used for export but not allocated", regIndex);
			...
			switch (regIndex)
			{
				...
				default:
				{
					printf("Microcode: Vertex shader register %d is out of usable export range", regIndex);
				}
			}
		}
	}
...
CodeExpr BlockTranslator::EmitVertexFetch(CodeExpr src, int fetchSlot, int fetchOffset, u32 stride, CXenonGPUMicrocodeTransformer::EFetchFormat format, bool isFloat, bool isSigned, bool isNormalized)
{
...
void BlockTranslator::EmitExec(const u32 codeAddr, CXenonGPUMicrocodeTransformer::EFlowBlock type, CodeStatement preamble, CodeStatement code, CodeExpr condition, const bool endOfShader)
{
	// no code to execute
	if (!code)
		return;
...
void BlockTranslator::EmitJump(const u32 targetAddr, CodeStatement preamble, CodeExpr condition)
{
	Block* block = new Block(condition.Get<ExprNode>(), targetAddr, Block::EBlockType::JUMP);
	m_createdBlocks.push_back(block);
}

void BlockTranslator::EmitCall(const u32 targetAddr, CodeStatement preamble, CodeExpr condition)
{
