...
void* CDX11StagingTexture::Lock( u32& outRowPitch, u32& outSlicePitch )
{
...
void CDX11StagingTexture::Flush( ID3D11Resource* target, const u32 targetSlice )
{
	// must be locked
	if ( !m_locked )
		return;
...
CDX11StagingTexture*CDX11StagingTexture::Create( ID3D11Device* dev, ID3D11DeviceContext* context, const u32 dims, DXGI_FORMAT format, const u32 width, const u32 height, const u32 depth )
{
	// invalid parameters
	if ( !dev || !context )
		return nullptr;
...
CDX11StagingTexture* CDX11StagingTextureCache::GetStagingTexture( const u32 dims, DXGI_FORMAT format, const u32 width, const u32 height, const u32 depth )
{
	...
	if ( !texture )
	{
		printf( "D3D: Failed to create staging texture, dim %d, size %dx%dx%d, format %d", dims, width, height, depth, format );
		return nullptr;
	}

	// add to registry
	printf( "D3D: Created staging texture, dim %d, size %dx%dx%d, format %d", dims, width, height, depth, format );
