...
u32 CDX11AbstractRenderTarget::GetMemoryPitch() const
{
	return m_sourcePitch;
}

int32_t CDX11AbstractRenderTarget::GetEDRAMPlacement() const
{
	return m_edramPlacement;
}

void CDX11AbstractRenderTarget::SetEDRAMPlacement(const int32_t placement)
{
...
u32 CDX11AbstractRenderTarget::GetFormatPixelSize(XenonColorRenderTargetFormat format)
{
	switch (format)
	{
...
u32 CDX11AbstractRenderTarget::GetMSSASampleCount(XenonMsaaSamples msaa)
{
	switch (msaa)
	{
...
u32 CDX11AbstractRenderTarget::GetFormatPixelSizeWithMSAA(XenonColorRenderTargetFormat format, XenonMsaaSamples msaa)
{
	const u32 basePixelSize = GetFormatPixelSize(format);
	const u32 samples = GetMSSASampleCount(msaa);
	return basePixelSize * samples;
}

CDX11AbstractRenderTarget* CDX11AbstractRenderTarget::Create(ID3D11Device* device, XenonColorRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch)
{
	// get the format
	...
	// compute width 
	const u32 pixelSize = GetFormatPixelSizeWithMSAA(format, msaa);
	const u32 expectedWidth = pitch;//  ;
	desc.Width = expectedWidth;

	// compute maximum height
	const u32 maxEDRAMSize = 10 * 1024 * 1024;
	const u32 maxHeight = 2560;// maxEDRAMSize / (pixelSize * pitch);
	...
	if (!tex || FAILED(hRet))
	{
		printf("D3D: Failed to create RT texture: 0x%X, width:%d, height:%d", hRet, desc.Width, desc.Height);
		return nullptr;
	}
	...
	{
		tex->Release();
		printf("D3D: Failed to create RTV for RTV texture: 0x%X, width:%d, height:%d", hRet, desc.Width, desc.Height);
		return nullptr;
	}
	...
	if (viewFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		if (!srv || FAILED(hRet))
		{
			rtv->Release();
			tex->Release();
			printf("D3D: Failed to create SRV for RT texture: 0x%X, width:%d, height:%d", hRet, desc.Width, desc.Height);
			return nullptr;
		}
	}
...
void CDX11AbstractDepthStencil::Clear(const bool clearDepth, const bool clearStencil, const float depthValue, const u32 stencilValue)
{
	DEBUG_CHECK(m_runtimeContext != nullptr);
...
u32 CDX11AbstractDepthStencil::GetMemoryPitch() const
{
	return m_sourcePitch;
}

int32_t CDX11AbstractDepthStencil::GetEDRAMPlacement() const
{
	return m_edramPlacement;
}

void CDX11AbstractDepthStencil::SetEDRAMPlacement(const int32_t placement)
{
	m_edramPlacement = placement;
}
...
u32 CDX11AbstractDepthStencil::GetFormatPixelSize(XenonDepthRenderTargetFormat format)
{
	switch (format)
	{
...
u32 CDX11AbstractDepthStencil::GetMSSASampleCount(XenonMsaaSamples msaa)
{
	switch (msaa)
	{
		case XenonMsaaSamples::MSSA1X: return 1;
...
u32 CDX11AbstractDepthStencil::GetFormatPixelSizeWithMSAA(XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa)
{
	const u32 basePixelSize = GetFormatPixelSize(format);
	const u32 samples = GetMSSASampleCount(msaa);
	return basePixelSize * samples;
...
CDX11AbstractDepthStencil* CDX11AbstractDepthStencil::Create(ID3D11Device* device, XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch)
{
	// setup texture desc
	D3D11_TEXTURE2D_DESC desc;
	memset(&desc, 0, sizeof(desc));
	...
	if (!dsv || FAILED(hRet))
	{
		tex->Release();
		texNonDepth->Release();
		printf("D3D: Failed to create DSV for DS texture: 0x%X, width:%d, height:%d", hRet, desc.Width, desc.Height);
		return nullptr;
	}
	...
	if (!srv || FAILED(hRet))
	{
		...
		tex->Release();
		texNonDepth->Release();
		printf("D3D: Failed to create SRV for DS texture: 0x%X, width:%d, height:%d", hRet, desc.Width, desc.Height);
		return nullptr;
	}
...
CDX11AbstractRenderTarget* CDX11SurfaceCache::GetRenderTarget(XenonColorRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch, const u32 edramPlacement)
{
...
CDX11AbstractDepthStencil* CDX11SurfaceCache::GetDepthStencil(XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch, const u32 edramPlacement)
{
	// find matching unused surface
...
CDX11AbstractRenderTarget* CDX11SurfaceCache::FindMatchingRT(XenonColorRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch)
{
...
CDX11AbstractDepthStencil* CDX11SurfaceCache::FindMatchingDS(XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch)
{
	// find unused RT matching requested structure
