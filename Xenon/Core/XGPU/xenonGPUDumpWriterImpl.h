...
public:
	...
	/// Block (recursive)
	virtual void BeginBlock( const char* tag, u32& outBlockID ) override;
	virtual void EndBlock( u32 blockId ) override;

	/// Packet (NON RECURSIVE)
	virtual void Packet( const u32 packetType, const void* dataWords, const unsigned int lengthInWords ) override;

	/// Register use of external memory for reading (memory will be restored just before this packed is executed)
	virtual void MemoryAccessRead( const u64 realAddress, const u32 size, const char* context ) override;

	/// Register use of external memory for writing (memory will be set to that state AFTER command was executed)
	virtual void MemoryAccessWrite( const u64 realAddress, const u32 size, const char* context ) override;
...
private:
	...
	typedef std::pair< u64, u64 > TMemoryKey; // crc, offset, size
	typedef std::map< TMemoryKey, u32 > TMemoryMap; // key -> block index

	// stack of blocks
	typedef std::vector< u32 > TBlockStack;
	...
	u32 MapMemoryBlock( const u64 address, const u32 size );
};
