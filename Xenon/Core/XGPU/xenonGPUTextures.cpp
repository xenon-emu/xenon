...
const u32 XenonTextureFormatInfo::GetBlockSizeInBytes() const
{
	return (m_blockHeight * m_blockWidth * m_bitsPerBlock) / 8;
}

const XenonTextureFormatInfo* XenonTextureFormatInfo::Get(const u32 gpuFormat)
{
...
const u32 XenonTextureInfo::CalculateMemoryRegionSize() const
{
...
void XenonTextureInfo::GetPackedTileOffset(u32& outOffsetX, u32& outOffsetY) const
{
	...
	if (std::min<u32> (m_size2D.m_logicalWidth, m_size2D.m_logicalHeight) > 16)
	{
		// Too big, not packed.
		outOffsetX = 0;
		outOffsetY = 0;
...
u32 XenonTextureInfo::TiledOffset2DOuter(const u32 y, const u32 width, const u32 log_bpp)
{
	const u32 macro = ((y >> 5) * (width >> 5)) << (log_bpp + 7);
	const u32 micro = ((y & 6) << 2) << log_bpp;
	return macro + ((micro & ~15) << 1) + (micro & 15) + ((y & 8) << (3 + log_bpp)) + ((y & 1) << 4);
}

// https://code.google.com/p/crunch/source/browse/trunk/inc/crn_decomp.h#4104
u32 XenonTextureInfo::TiledOffset2DInner(const u32 x, const u32 y, const u32 bpp, const u32 baseOffset)
{
	// copy pasted....
	const u32 macro = (x >> 5) << (bpp + 7);
	const u32 micro = (x & 7) << bpp;
	const u32 offset = baseOffset + (macro + ((micro & ~15) << 1) + (micro & 15));
	return ((offset & ~511) << 3) + ((offset & 448) << 2) + (offset & 63) + ((y & 16) << 7) + (((((y & 8) >> 2) + (x >> 3)) & 3) << 6);
}

const u64 XenonTextureInfo::GetHash() const
{
	return 0;
}
...
void XenonTextureInfo::CalculateTextureSizes2D(const XenonGPUTextureFetch& fetch)
{
	...
	const u32 blockWidth = Helper::RoundUp(m_size2D.m_logicalWidth, m_format->m_blockWidth) / m_format->m_blockWidth;
	const u32 blockHeight = Helper::RoundUp(m_size2D.m_logicalHeight, m_format->m_blockHeight) / m_format->m_blockHeight;

	// Tiles are 32x32 blocks. All textures must be multiples of tile dimensions.
	const u32 tileWidth = (u32)(std::ceilf(blockWidth / 32.0f));
	const u32 tileHeight = (u32)(std::ceilf(blockHeight / 32.0f));
	m_size2D.m_actualBlockWidth = tileWidth * 32;
	m_size2D.m_actualBlockHeight = tileHeight * 32;

	const u32 bytePerBlock = (m_format->m_blockWidth * m_format->m_blockHeight * m_format->m_bitsPerBlock) / 8;
	u32 bytePitch = tileWidth * 32 * bytePerBlock;
...
void XenonTextureInfo::CalculateTextureSizesCube(const XenonGPUTextureFetch& fetch)
{
	...
	// w/h in blocks must be a multiple of block size.
	const u32 blockWidth = Helper::RoundUp(m_sizeCube.m_logicalWidth, m_format->m_blockWidth) / m_format->m_blockWidth;
	const u32 blockHeight = Helper::RoundUp(m_sizeCube.m_logicalHeight, m_format->m_blockHeight) / m_format->m_blockHeight;

	// Tiles are 32x32 blocks. All textures must be multiples of tile dimensions.
	const int32_t tileWidth = (u32)(std::ceilf(blockWidth / 32.0f));
	const int32_t tileHeight = (u32)(std::ceilf(blockHeight / 32.0f));
	m_sizeCube.m_actualBlockWidth = tileWidth * 32;
	m_sizeCube.m_actualBlockHeight = tileHeight * 32;

	const u32 bytesPerBlock = (m_format->m_blockWidth * m_format->m_blockHeight * m_format->m_bitsPerBlock) / 8;
	u32 bytePitch = tileWidth * 32 * bytesPerBlock;
...
u32 XenonSamplerInfo::GetHash() const
{
	u32 hash = 0;
	hash = (hash * 4) + (u32)m_minFilter;
	hash = (hash * 4) + (u32)m_magFilter;
	hash = (hash * 4) + (u32)m_mipFilter;
	hash = (hash * 8) + (u32)m_clampU;
	hash = (hash * 8) + (u32)m_clampV;
	hash = (hash * 8) + (u32)m_clampW;
	hash = (hash * 4) + (u32)m_borderColor;
	hash = (hash * 4) + (u32)m_anisoFilter;
	hash ^= (u32&)m_lodBias;
	return hash;
}
