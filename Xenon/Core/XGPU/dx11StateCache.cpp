...
bool CDX11StateCache::SetDepthStencil( const D3D11_DEPTH_STENCIL_DESC& dsState, const u32 stencilRef )
{
	// compute input hash
	...
	if ( FAILED(hRet) || !ds )
	{
		printf( "D3D: Failed to create DepthStencil state from provided input, error = 0x%08X", hRet );
		return false;
	}
...
bool CDX11StateCache::SetBlendState( const D3D11_BLEND_DESC& blendState, const float blendFactors[4] )
{
	...
	if ( FAILED(hRet) || !bs )
	{
		printf( "D3D: Failed to create Blend state from provided input, error = 0x%08X", hRet );
		return false;
	}
...
bool CDX11StateCache::SetRasterState( const D3D11_RASTERIZER_DESC& rasterState )
{
	...
	if ( FAILED(hRet) || !rs )
	{
		printf( "D3D: Failed to create Rasterizer state from provided input, error = 0x%08X", hRet );
		return false;
	}
