...
public:
	...
	// RT&DS interface
	void BindColorRenderTarget( const u32 index, const XenonColorRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch );
	void BindDepthStencil( const XenonDepthRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch );	
	void UnbindColorRenderTarget( const u32 index );
	void UnbindDepthStencil();

	/// Realize setup - this will actually allocate the resource and prepare for rendering
	bool RealizeSetup( u32& outMainWidth,  u32& outMainHeight );

	/// Clear
	void ClearColor( const u32 index, const float* clearColor, const bool flushToEDRAM );
	void ClearDepth( const float depthClear, const u32 stencilClear, const bool flushToEDRAM );

	/// Copy out data from EDRAM
	bool ResolveColor(const u32 srcIndex, const XenonColorRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, CDX11AbstractTexture* destTexture, const XenonRect2D& destRect);

	/// Copy out the depth data from EDRAM
	bool ResolveDepth(const XenonDepthRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, CDX11AbstractTexture* destTexture, const XenonRect2D& destRect);
...
#pragma pack(push,4)
	struct Settings
	{
		int32_t m_copySrcOffsetX;
		int32_t m_copySrcOffsetY;
		int32_t m_copyDestOffsetX;
		int32_t m_copyDestOffsetY;
		int32_t m_copyDestSizeX;
		int32_t m_copyDestSizeY;
