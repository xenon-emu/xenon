...
static const std::unordered_map<u8, const std::string_view> opcodeNames = {
{ 0x10, "PM4_NOP" },
...
static inline std::string_view GetPM4Opcode(u8 opcode) {
  auto it = opcodeNames.find(opcode);
  return it != opcodeNames.end() ? it->second : "UNK_OP";
}
