...
CDX11MicrocodeShader* CDX11MicrocodeShader::ExtractPixelShader(const void* shaderCode, const u32 shaderCodeSize, const u64 microcodeHash)
{
...
CDX11MicrocodeShader* CDX11MicrocodeShader::ExtractVertexShader(const void* shaderCode, const u32 shaderCodeSize, const u64 microcodeHash)
{
...
u32 CDX11MicrocodeShader::GetFetchFormatChannels(const EFetchFormat format)
{
	switch (format)
	{
...
{
	const u32 numFetches = m_decompiledShader->GetNumVertexFetches();
	fetches.reserve(numFetches);
	for (u32 i = 0; i < numFetches; ++i)
	{
		...
		info.m_layoutSlot = (u16)fetches.size();
...
{
	const u32 numTextures = m_decompiledShader->GetNumUsedTextures();
...
{
	const u32 numExports = m_decompiledShader->GetNumExportInstructions();
...
void CDX11MicrocodeShader::GetUsedRegisters(std::vector< u32 >& outUsedRegs) const
{
	outUsedRegs.resize(m_decompiledShader->GetNumUsedRegisters());
	for (u32 i = 0; i < m_decompiledShader->GetNumUsedRegisters(); ++i)
	{
		outUsedRegs[i] = m_decompiledShader->GetUsedRegister(i);
	}
}

u32 CDX11MicrocodeShader::GetEntryPointAddress() const
{
	return m_decompiledShader->GetCode()->GetEntryPointAddress();
}

u32 CDX11MicrocodeShader::GetNumUsedInterpolators() const
{
	return m_decompiledShader->GetNumUsedInterpolators();
}

u32 CDX11MicrocodeShader::GetTextureFetchSlotMask() const
{
...
void CDX11MicrocodeShader::DumpShader(const std::wstring& absoluteFilePath) const
{
	...
	if (m_decompiledShader->GetNumExportInstructions() > 0)
	{
		...
		for (u32 i = 0; i < m_decompiledShader->GetNumExportInstructions(); ++i)
		{
			const auto* instr = m_decompiledShader->GetExportInstruction(i);
	...
	if (m_decompiledShader->GetNumVertexFetches() > 0)
	{
		...
		for (u32 i = 0; i < m_decompiledShader->GetNumVertexFetches(); ++i)
		{
			const auto* instr = m_decompiledShader->GetVertexFetch(i);
	...
	{
		const u32 numBlocks = m_decompiledShader->GetCode()->GetNumBlocks();
		fprintf(f, "%d blocks\n\n", numBlocks);
