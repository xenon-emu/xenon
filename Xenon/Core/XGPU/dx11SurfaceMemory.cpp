	...
	, m_clear32("Clear32")
{
	...
	const u32 edramMemorySize = NUM_EDRAM_TILES * EDRAM_TILE_X * EDRAM_TILE_Y * EDRAM_BPP;
	printf("EDRAM: Calculated memory size = %d", edramMemorySize);
	...
	// create EDRAM memory buffer
	{
		...
		{
			printf("EDRAM: Failed to create EDRAM memory buffer. Error = 0x%08X", hRet);
		}
	}
	...
	// CPU access buffer
	{
		...
		if (!m_edramMemory || FAILED(hRet))
		{
			printf("EDRAM: Failed to create EDRAM copy buffer. Error = 0x%08X", hRet);
		}
	}
	...
	// create resource view
	{
		...
		if (!m_edramView || FAILED(hRet))
		{
			...
			printf("EDRAM: Failed to create EDRAM view buffer. Error = 0x%08X", hRet);
		}
	}
...
const u32 CDX11SurfaceMemory::TranslateToGenericFormat_EDRAM(const XenonColorRenderTargetFormat format)
{
	switch (format)
	{
	...
	printf("Unsupported EDRAM format: %u", format);
	DEBUG_CHECK(!"Unsupported render target format for EDRAM");
	return 0;
}

const u32 CDX11SurfaceMemory::TranslateToGenericFormat_EDRAM(const XenonDepthRenderTargetFormat format)
{
	DEBUG_CHECK(!"Unsupported depth stencil format for EDRAM");
	return 0;
}

const u32 CDX11SurfaceMemory::TranslateToGenericFormat_Surface(const XenonColorRenderTargetFormat format)
{
	switch (format)
	{
		case XenonColorRenderTargetFormat::Format_8_8_8_8: return GENERIC_FORMAT_8_8_8_8;
...
const u32 CDX11SurfaceMemory::TranslateToGenericFormat_Surface(const XenonDepthRenderTargetFormat format)
{
	DEBUG_CHECK(!"Unsupported depth stenicil format for surface");
	return 0;
}

const u32 CDX11SurfaceMemory::TranslateToGenericFormat_Surface(const XenonTextureFormat format)
{
	switch (format)
	{
...
void CDX11SurfaceMemory::CopyIntoEDRAM(class CDX11AbstractRenderTarget* rt)
{
	...
	{
		//printf( "Non MSAA1 render target: [%dx%d], EDRAM 0x%04X, Format %hs", rt->GetPhysicalWidth(), rt->GetPhysicalHeight(), rt->GetEDRAMPlacement(), XenonGPUGetColorRenderTargetFormatName( rt->GetFormat() ) );
		return;
	}
	...
	const u32 maxBlocks = 1024 * 10;
	const u32 tailingBlocks = maxBlocks - rt->GetEDRAMPlacement();
	const u32 tailingHeight = (tailingBlocks * 1024) / rt->GetMemoryPitch();

	// determine copy placement
	const u32 copyX = 0;
	const u32 copyY = 0;
	const u32 copyWidth = std::min<u32>(texDesc.Width, rt->GetMemoryPitch());
	const u32 copyHeight = std::min<u32>(texDesc.Height, tailingHeight);
...
void CDX11SurfaceMemory::ClearInEDRAM(const XenonColorRenderTargetFormat format, const u32 edramBase, const u32 edramPitch, const u32 height, const float* clearColor)
{
	// configure EDRAM placement
...
void CDX11SurfaceMemory::CopyFromEDRAM(class CDX11AbstractRenderTarget* rt)
{
	...
	// limit copiable size
	u32 maxCopyWidth = copyWidth;
	maxCopyWidth = std::min<u32>(maxCopyWidth, texDesc.Width - copyX);
	u32 maxCopyHeight = copyHeight;
	maxCopyHeight = std::min<u32>(maxCopyHeight, texDesc.Height - copyY);
...
void CDX11SurfaceMemory::CopyIntoEDRAM(class CDX11AbstractDepthStencil* ds)
{
	...
	// number of EDRAM blocks left - that's how much we need to copy
	const u32 maxBlocks = 1024 * 10;
	const u32 tailingBlocks = maxBlocks - ds->GetEDRAMPlacement();
	const u32 tailingHeight = (tailingBlocks * 1024) / ds->GetMemoryPitch();

	// determine copy placement
	const u32 copyX = 0;
	const u32 copyY = 0;
	const u32 copyWidth = std::min<u32>(ds->GetPhysicalWidth(), ds->GetMemoryPitch());
	const u32 copyHeight = std::min<u32>(ds->GetPhysicalHeight(), tailingHeight);
...
void CDX11SurfaceMemory::Resolve(const XenonColorRenderTargetFormat srcFormat, const u32 srcBase, const u32 srcPitch, const u32 srcX, const u32 srcY, const u32 destX, const u32 destY, const u32 width, const u32 height, class CDX11AbstractSurface* destSurf)
{
	// argument check
	...
	// limit copiable size
	u32 maxCopyWidth = width;
	maxCopyWidth = std::min<u32>(maxCopyWidth, destSurf->GetWidth() - destX);
	u32 maxCopyHeight = height;
	maxCopyHeight = std::min<u32>(maxCopyHeight, destSurf->GetHeight() - destY);
...
#undef char
	...
	printf("EDRAM: All shaders loaded");
	return true;
}

void CDX11SurfaceMemory::DispatchFlat(const CDX11ComputeShader& shader, const u32 memoryOffset, const u32 memorySize)
{
	...
	const u32 numPixels = memorySize / 4;
	const u32 numBlocks = numPixels / 1024;
...
void CDX11SurfaceMemory::DispatchPixels(const CDX11ComputeShader& shader, const u32 width, const u32 height, ID3D11UnorderedAccessView* additionalUAV)
{
	...
	// calculate number of blocks
	const u32 numBlocksX = width / 8;
	const u32 numBlocksY = height / 8;
...
void CDX11SurfaceMemory::DispatchPixels(const CDX11ComputeShader& shader, const u32 width, const u32 height, ID3D11ShaderResourceView* shaderView)
{
