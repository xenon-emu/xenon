...
public:
	...
	virtual u32 GetMemoryPitch() const override final;
	virtual int32_t GetEDRAMPlacement() const override final;
	virtual void Clear(const float* clearColor) override final;

	// Get physical texture width
	inline const u32 GetPhysicalWidth() const { return m_physicalWidth; }
	inline const u32 GetPhysicalHeight() const { return m_physicalHeight; }
	...
	/// EDRAM placement
	void SetEDRAMPlacement(const int32_t placement);

	/// translate format
	static void TranslateFormat(XenonColorRenderTargetFormat format, DXGI_FORMAT& outTextureFormat, DXGI_FORMAT& outViewFormat);


	static u32 GetFormatPixelSize(XenonColorRenderTargetFormat format);
	static u32 GetFormatPixelSizeWithMSAA(XenonColorRenderTargetFormat format, XenonMsaaSamples msaa);
	static u32 GetMSSASampleCount(XenonMsaaSamples msaa);

	/// Create RT surface
	static CDX11AbstractRenderTarget* Create(ID3D11Device* device, XenonColorRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch);
...
private:
	...
	int32_t							m_edramPlacement;

	u32							m_physicalWidth;
	u32							m_physicalHeight;
...
public:
	...
	virtual u32 GetMemoryPitch() const override final;
	virtual int32_t GetEDRAMPlacement() const override final;
	virtual void Clear(const bool clearDepth, const bool clearStencil, const float depthValue, const u32 stencilValue) override final;

	// Get physical texture width
	inline const u32 GetPhysicalWidth() const { return m_physicalWidth; }
	inline const u32 GetPhysicalHeight() const { return m_physicalWidth; }
	...
	static u32 GetFormatPixelSize(XenonDepthRenderTargetFormat format);
	static u32 GetFormatPixelSizeWithMSAA(XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa);
	static u32 GetMSSASampleCount(XenonMsaaSamples msaa);

	/// Create RT surface
	static CDX11AbstractDepthStencil* Create(ID3D11Device* device, XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch);
...
public:
	...
	CDX11AbstractRenderTarget* GetRenderTarget(XenonColorRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch, const u32 placement);

	/// Get usable depth stencil surface matching specified parameters
	CDX11AbstractDepthStencil* GetDepthStencil(XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch, const u32 placement);
...
private:
	...
	CDX11AbstractRenderTarget* FindMatchingRT(XenonColorRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch);

	// Find matching DS that is not assigned into EDRAM, returns NULL if nothing free found
	CDX11AbstractDepthStencil* FindMatchingDS(XenonDepthRenderTargetFormat format, XenonMsaaSamples msaa, const u32 pitch);
};
