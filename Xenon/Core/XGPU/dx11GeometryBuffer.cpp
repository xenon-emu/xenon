...
CDX11GeometryBuffer::LinearAllocator::LinearAllocator( const u32 size )
	: m_offset(0)
	, m_size(size)
{
}
...
u32 CDX11GeometryBuffer::LinearAllocator::Alloc( const u32 size, bool& wrapped )
{
...
bool CDX11GeometryBuffer::AllocStagingBuffer( const u32 sourceMemorySize, void*& outPtr, u32& outOffset )
{
	DEBUG_CHECK( (sourceMemorySize & 3) == 0 );	

	// allocate space in the vertex cache
	bool vertexDataWrapped = false;
	const u32 vertexDataOffset = m_geometryDataAllocator.Alloc( sourceMemorySize, vertexDataWrapped );
...
CDX11GeometryBuffer::BufferHandle CDX11GeometryBuffer::UploadStagingData( const u32 sourceMemorySize, const u32 dataOffset, const u32 type )
{
	// umap
...
const CDX11GeometryBuffer::BufferHandle CDX11GeometryBuffer::UploadVertices( const void* sourceMemory, const u32 sourceMemorySize, const bool verticesNeedSwapping )
{
	...
	u32* dest = (u32*) stagingPtr;
	const u32* src = (const u32*) sourceMemory;
	if ( verticesNeedSwapping )
	{
		for ( u32 i=0; i<(sourceMemorySize/4); ++i )
		{
...
const CDX11GeometryBuffer::BufferHandle CDX11GeometryBuffer::UploadIndices16( const void* sourceMemory, const u32 numIndices, const bool indicesNeedSwapping )
{
	DEBUG_CHECK( sourceMemory != nullptr );

	// count size of memory
	const u32 sourceMemorySize = numIndices * sizeof(u32);
	...
	// copy data SWAPPING it along the way
	u32* dest = (u32*) stagingPtr;
	const u16* src = (const u16*) sourceMemory;
...
const CDX11GeometryBuffer::BufferHandle CDX11GeometryBuffer::UploadIndices32( const void* sourceMemory, const u32 numIndices, const bool indicesNeedSwapping )
{
	DEBUG_CHECK( sourceMemory != nullptr );
...
const bool CDX11GeometryBuffer::BindData( BufferHandle data, const u32 bufferIndex )
{
