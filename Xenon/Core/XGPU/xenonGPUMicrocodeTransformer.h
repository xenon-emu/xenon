...
public:
	...
	public:
		...
		// vertex data fetch
		virtual CodeExpr EmitVertexFetch( CodeExpr src, int fetchSlot, int fetchOffset, u32 stride, EFetchFormat format, bool isFloat, bool isSigned, bool isNormalized ) = 0;
		...
		virtual void EmitExec( const u32 codeAddr, EFlowBlock type, CodeStatement preamble, CodeStatement code, CodeExpr condition, const bool endOfShader ) = 0;
		virtual void EmitJump( const u32 targetAddr, CodeStatement preamble, CodeExpr condition ) = 0;
		virtual void EmitCall( const u32 targetAddr, CodeStatement preamble, CodeExpr condition ) = 0;
	...
	/// Decompile shader
	void TransformShader( ICodeWriter& writer, const u32* words, const u32 numWords );

private:
	XenonShaderType			m_shaderType;
	u32					m_lastVertexStride;

	static const u32 GetArgCount( EVectorInstr instr );
	static const u32 GetArgCount( EScalarInstr instr );

	void TransformBlock( ICodeWriter& writer, const u32* words, const u32 numWords, const ucode::instr_cf_t* cf, u32& codePC );

	CodeStatement EmitExec( ICodeWriter& writer, const u32* words, const u32 numWords, const ucode::instr_cf_exec_t& exec );
	...
	CodeStatement EmitPredicateTest( ICodeWriter& writer, const CodeStatement& code, const bool bIsConditional, const u32 flowPredCondition, const u32 predSelect, const u32 predCondition );

	CodeExpr EmitSrcReg( ICodeWriter& writer, const ucode::instr_alu_t& op, const u32 num, const u32 type, const u32 swiz, const u32 negate, const int constSlot );
	CodeExpr EmitSrcReg( ICodeWriter& writer, const ucode::instr_alu_t& op, const u32 argIndex );
