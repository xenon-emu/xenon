...
void CDX11SurfaceManager::EDRAMSetup::Dump()
{
	...
	if ( m_depth.IsValid() )
	{
		printf( "EDRAM:  Depth: %hs, %hs, pitch:%d, base: %d", 
	...
	for ( u32 i=0; i<4; ++i )
	{
		...
		if ( color.IsValid() )
		{
			printf( "EDRAM: Color%d: %hs, %hs, pitch:%d, base: %d", 
				i,
...
void CDX11SurfaceManager::BindColorRenderTarget(const u32 index, const XenonColorRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch)
{
	DEBUG_CHECK(index < 4);
	DEBUG_CHECK(pitch != 0);
	...
	data.m_msaa = msaa;
	data.m_base = base;
	data.m_pitch = std::max<u32>(2560, pitch);
}

void CDX11SurfaceManager::BindDepthStencil(const XenonDepthRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch)
{
	DEBUG_CHECK(pitch != 0);
...
void CDX11SurfaceManager::UnbindColorRenderTarget(const u32 index)
{
	DEBUG_CHECK(index < 4);
...
bool CDX11SurfaceManager::RealizeSetup(u32& outMainWidth, u32& outMainHeight)
{
	...
	std::vector<u32> usedEDRAMOffsets;
	{
...
bool CDX11SurfaceManager::ResolveColor(const u32 srcIndex, const XenonColorRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, CDX11AbstractTexture* destTexture, const XenonRect2D& destRect)
{
	...
	if (rt == nullptr)
	{
		printf("EDRAM: No color surface at RT%d to copy to EDRAM", srcIndex);
		return false;
	}
	...
	{
		printf("EDRAM: Color surface at RT%d not bound to EDRAM", srcIndex);
		return false;
	}
...
bool CDX11SurfaceManager::ResolveDepth(const XenonDepthRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, CDX11AbstractTexture* destTexture, const XenonRect2D& destRect)
{
	// destiation texture MUST be given
	DEBUG_CHECK(destTexture != nullptr);
	if (!destTexture)
	...
	if (ds == nullptr)
	{
		printf("EDRAM: No depth surface to copy from EDRAM");
		return false;	
	}
	...
	{
		printf("EDRAM: Depth surface not bound to EDRAM");
		return false;
	}
	...
	printf("Resolve: Depth resolve not supported");
	return false;
}

void CDX11SurfaceManager::ClearColor(const u32 index, const float* clearColor, const bool flushToEDRAM)
{
	DEBUG_CHECK(index < 4);
...
void CDX11SurfaceManager::ClearDepth(const float depthClear, const u32 stencilClear, const bool flushToEDRAM)
{
	// clear the surface if bounded
