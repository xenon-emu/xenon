...
namespace Xe::XGPU {
  ...
  CommandProcessor::CommandProcessor(RAM* ramPtr, XenosState* statePtr, Render::Renderer* renderer,
                                     PCIBridge* pciBridge)
      : ram(ramPtr)
      , state(statePtr)
      ,
  ...
  CommandProcessor::~CommandProcessor() {
    ...
    if (cpWorkerThread.joinable()) { cpWorkerThread.join(); }
  }
  ...
  void CommandProcessor::CPUpdateRBSize(size_t newSize) {
    if (!(newSize & CP_RB_CNTL_RB_BUFSZ_MASK)) return;

    cpRingBufferSize = static_cast<size_t>(1u << (newSize + 3));
    LOG_DEBUG(Xenos, "CP: Updating RingBuffer Size: 0x{:X}", cpRingBufferSize.load());
  }

  void CommandProcessor::CPUpdateRBWritePointer(u32 offset) { cpWritePtrIndex = offset; }
  ...
  void CommandProcessor::cpWorkerThreadLoop() {
    ...
    while (cpWorkerThreadRunning) {
      ...
      if (!cpWorkerThreadRunning) break;
  ...
  void CommandProcessor::CPSetSQProgramCntl(u32 value) { state->programCntl = value; }
  ...
  bool CommandProcessor::ExecutePacketType0(RingBuffer* ringBuffer, u32 packetData) {
    ...
    for (u64 idx = 0; cpWorkerThreadRunning && idx < regCount; idx++) {
      ...
      // LOG_DEBUG(Xenos, "CP[ExecutePacketType0]: Writing to {} (0x{:X}), data 0x{:X}",
      // Xe::XGPU::GetRegisterNameById(targetRegIndex), targetRegIndex, registerData);
  ...
  bool CommandProcessor::ExecutePacketType3(RingBuffer* ringBuffer, u32 packetData) {
    ...
    switch (currentOpCode) {
      case Xe::XGPU::PM4_NOP: result = ExecutePacketType3_NOP(ringBuffer, packetData, dataCount); break;
      case Xe::XGPU::PM4_REG_RMW: result = ExecutePacketType3_REG_RMW(ringBuffer, packetData, dataCount); break;
      case Xe::XGPU::PM4_DRAW_INDX: result = ExecutePacketType3_DRAW_INDX(ringBuffer, packetData, dataCount); break;
      case Xe::XGPU::PM4_IM_LOAD: result = ExecutePacketType3_IM_LOAD(ringBuffer, packetData, dataCount); break;
      ...
      case Xe::XGPU::PM4_SET_CONSTANT:
        ...
        break;
      case Xe::XGPU::PM4_DRAW_INDX_2: result = ExecutePacketType3_DRAW_INDX_2(ringBuffer, packetData, dataCount); break;
      ...
      case Xe::XGPU::PM4_WAIT_REG_MEM:
        ...
        break;
      case Xe::XGPU::PM4_COND_WRITE: result = ExecutePacketType3_COND_WRITE(ringBuffer, packetData, dataCount); break;
      case Xe::XGPU::PM4_EVENT_WRITE: result = ExecutePacketType3_EVENT_WRITE(ringBuffer, packetData, dataCount); break;
      case Xe::XGPU::PM4_EVENT_WRITE_EXT:
        result = ExecutePacketType3_EVENT_WRITE_EXT(ringBuffer, packetData, dataCount);
        break;
      case Xe::XGPU::PM4_ME_INIT: result = ExecutePacketType3_ME_INIT(ringBuffer, packetData, dataCount); break;
      ...
      case Xe::XGPU::PM4_SET_BIN_SELECT:
        ...
        break;
      case Xe::XGPU::PM4_INTERRUPT: result = ExecutePacketType3_INTERRUPT(ringBuffer, packetData, dataCount); break;
  ...
  bool CommandProcessor::ExecutePacketType3_EVENT_WRITE_EXT(RingBuffer* ringBuffer, u32 packetData, u32 dataCount) {
    // Generates an event that creates a write to memory when completed
    const u32 initiator = ringBuffer->ReadAndSwap<u32>();
    ...
    auto endianness = static_cast<Xe::eEndianFormat>(address & 0x3);
    address &= ~0x3;
    ...
    u16 extents[] = {
      0 >> 3,                             // min x
      xeTexture2DCubeMaxWidthHeight >> 3, // max x
      0 >> 3,                             // min y
      xeTexture2DCubeMaxWidthHeight >> 3, // max y
      0,                                  // min z
      1,                                  // max z
    };
    ...
    for (size_t i = 0; i < 6; i++) { extents[i] = byteswap_be<u16>(extents[i]); }
    memcpy(addrPtr, &extents, sizeof(extents));
    return true;
  }
  ...
  bool CommandProcessor::ExecutePacketType3_ME_INIT(RingBuffer* ringBuffer, u32 packetData, u32 dataCount) {
    ...
    for (size_t i = 0; i < dataCount; i++) { cpME_PM4_ME_INIT_Data.push_back(ringBuffer->ReadAndSwap<u32>()); }
    return true;
  }
  ...
  public:
    virtual void OnExprStart(Microcode::AST::ExpressionNode::Ptr n) override final {
      if (n->GetType() == Microcode::AST::eExprType::VFETCH) {
        ...
        if (regIndex != -1) { usedRegisters.insert((u32)regIndex); }
      }
    }
  ...
  public:
    ...
    virtual void OnWrite(Microcode::AST::ExpressionNode::Ptr dest, Microcode::AST::ExpressionNode::Ptr src,
                         std::array<eSwizzle, 4> mask) override final {
  ...
  void VisitAll(const Microcode::AST::Block* b, Microcode::AST::StatementNode::Visitor& v) {
    if (b->GetCondition()) v.OnConditionPush(b->GetCondition()->shared_from_this());

    if (b->GetCode()) b->GetCode()->Visit(v);

    if (b->GetCondition()) v.OnConditionPop();
  }
  ...
  std::pair<Microcode::AST::Shader*, std::vector<u32>> LoadShader(eShaderType shaderType, const std::vector<u32>& data,
    ...
    Microcode::AST::Shader* shader = Microcode::AST::Shader::DecompileMicroCode(
      reinterpret_cast<const u8*>(data.data()), data.size() * 4, shaderType);
#ifndef NO_GFX
    Microcode::AST::ShaderCodeWriterSirit writer{shaderType};
    if (shader) { shader->EmitShaderCode(writer); }
  ...
  bool CommandProcessor::ExecutePacketType3_IM_LOAD(RingBuffer* ringBuffer, u32 packetData, u32 dataCount) {
    ...
    LOG_DEBUG(Xenos, "[CP::IM_LOAD] Shader Address: 0x{:X} | Shader Size: 0x{:X} (0x{:X}, 0x{:X})", addr, startSize,
              start, size);
    ...
    for (u32& value : data) { value = byteswap_be(value); }
    ...
    switch (shaderType) {
      ...
      case eShaderType::Pixel: {
#ifndef NO_GFX
        {
          ...
          if (shaderType == eShaderType::Pixel) render->pendingPixelShaders[crc] = shader;
          else render->pendingVertexShaders[crc] = shader;
        }

        Render::RenderCommand cmd{};
        cmd.type = Render::RenderCommandType::BindShader;
        cmd.payload = Render::RenderCommand::BindShaderCmd{.vsHash = (shaderType == eShaderType::Vertex ? crc : 0),
                                                           .psHash = (shaderType == eShaderType::Pixel ? crc : 0)};
        LOG_DEBUG(Xenos, "[CP::IM_LOAD] {}Shader CRC: 0x{:08X}", shaderType == eShaderType::Pixel ? "Pixel" : "Vertex",
                  crc);
  ...
  bool CommandProcessor::ExecutePacketType3_IM_LOAD_IMMEDIATE(RingBuffer* ringBuffer, u32 packetData, u32 dataCount) {
    ...
    switch (shaderType) {
      ...
      case eShaderType::Pixel: {
        ...
        cmd.payload = Render::RenderCommand::BindShaderCmd{.vsHash = (shaderType == eShaderType::Vertex ? crc : 0),
                                                           .psHash = (shaderType == eShaderType::Pixel ? crc : 0)};
        LOG_DEBUG(Xenos, "[CP::IM_LOAD_IMMEDIATE] {}Shader CRC: 0x{:08X}",
                  shaderType == eShaderType::Pixel ? "Pixel" : "Vertex", crc);
  ...
  bool CommandProcessor::ExecutePacketType3_INDIRECT_BUFFER(RingBuffer* ringBuffer, u32 packetData, u32 dataCount) {
    ...
    LOG_TRACE(Xenos, "CP[IndirectBuffer]: Executing indirect buffer at address 0x{:X}, size 0x{:X}", bufferPtr,
              bufferSize);
  ...
  bool CommandProcessor::ExecutePacketType3_SET_SHADER_CONSTANTS(RingBuffer* ringBuffer, u32 packetData,
                                                                 u32 dataCount) {
    // Get base index
    const u32 offsetType = ringBuffer->ReadAndSwap<u32>();
    u32 index = offsetType & 0xFFFF;
      ...
      if (isMemory) {
        ...
        if (static_cast<XeRegister>(pollReg) == XeRegister::COHER_STATUS_HOST) {
          ...
          if (statusHost.status) {
            ...
            LOG_DEBUG(Xenos, "[CP]: Making {:#x} -> {:#x} coherent, performed action = {}", baseHost,
                      baseHost + sizeHost, action);
  ...
  bool CommandProcessor::ExecutePacketType3_DRAW(RingBuffer* ringBuffer, u32 packetData, u32 dataCount,
                                                 u32 vizQueryCondition, const char* opCodeName) {
    // Sanity check.
    if (!dataCount) {
    ...
    switch (state->vgtDrawInitiator.sourceSelect) {
      ...
      case eSourceSelect::xeImmediate: {
        ...
        LOG_ERROR(Xenos,
                  "[CP, PT3]{}: Is using immediate vertex indices, which are currently unsupported. "
                  "Please submit an issue in Xenon github.",
                  opCodeName);
        drawOk = false;
      } break;
...
#endif
#ifndef NO_GFX
      LOG_DEBUG(Xenos, "[CP] Draw {}: PrimType {}, IndexCount {}", isIndexedDraw ? "Indexed" : "Auto",
                (u32)state->vgtDrawInitiator.primitiveType, state->vgtDrawInitiator.numIndices);
#else
      LOG_DEBUG(Xenos, "[CP] Draw {}: PrimType {}, IndexCount {}", isIndexedDraw ? "Indexed" : "Auto",
                (u32)state->vgtDrawInitiator.primitiveType, state->vgtDrawInitiator.numIndices);
#endif
  ...
  bool CommandProcessor::ExecutePacketType3_LOAD_ALU_CONSTANT(RingBuffer* ringBuffer, u32 packetData, u32 dataCount) {
    ...
    switch (type) {
      ...
      case 4: // REGISTERS
        index += 0x2000;
        break;
      default: LOG_ERROR(Xenos, "[CP] [PT3:LOAD_ALU_CONSTANT]: Unrecognized Constant Type provided."); return true;
    }
    ...
    for (u32 n = 0; n < sizeInDwords; n++, index++) {
      ...
      cmd.payload = Render::RenderCommand::UploadBufferCmd{
        "ALUConsts"_j, {}, Render::eBufferType::Storage, Render::eBufferUsage::DynamicDraw};
