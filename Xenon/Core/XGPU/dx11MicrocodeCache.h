...
public:
	...
	CDX11MicrocodeShader* GetCachedPixelShader( const void* shaderCode, const u32 shaderCodeSize );

	/// Decompile vertex shader (can use cached version)
	CDX11MicrocodeShader* GetCachedVertexShader( const void* shaderCode, const u32 shaderCodeSize );

private:
	typedef u64 TMicrocodeHash;
	typedef std::map< TMicrocodeHash, CDX11MicrocodeShader* >	TMicrocodeMap;

	static TMicrocodeHash ComputeHash( const void* shaderCode, const u32 shaderCodeSize );
