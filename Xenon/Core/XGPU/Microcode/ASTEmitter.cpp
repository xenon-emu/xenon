...
namespace Xe::Microcode::AST {
  ...
  void ShaderCodeWriterSirit::FinalizeEntryPoint() {
    ...
    spv::ExecutionModel execModel
      = type == eShaderType::Pixel ? spv::ExecutionModel::Fragment : spv::ExecutionModel::Vertex;
    if (type == eShaderType::Pixel) {
      module.AddExecutionMode(main_func, spv::ExecutionMode::OriginUpperLeft);
    } else {
      for (const auto& [slot, var] : vertex_input_vars) { interface_vars.push_back(var); }
    }
  ...
  void ShaderCodeWriterSirit::EndMain() {
    ...
    if (entry_dispatch_func.value) { module.OpFunctionCall(module.TypeVoid(), entry_dispatch_func, {}); }
    Sirit::Id float_type = module.TypeFloat(32);
    Sirit::Id vec4_type = module.TypeVector(float_type, 4);
    module.OpReturn();
  ...
  Chunk ShaderCodeWriterSirit::GetConstantReg(u32 regIndex) {
    ...
    Sirit::Id ptr_to_constant = module.OpAccessChain(module.TypePointer(spv::StorageClass::Uniform, vec4_type),
                                                     ubo_var_v, zero_const, reg_index_const);
  ...
  Chunk ShaderCodeWriterSirit::Swizzle(const Chunk& value, std::array<eSwizzle, 4> swizzle) {
    ...
    if (value.type == eChunkType::Scalar) {
      ...
      for (u8 i = 0; i != 4; ++i) { scalar_comps[i] = value.id; }
    ...
    for (u8 i = 0; i != 4; ++i) { components[i] = static_cast<u32>(swizzle[i]); }
    return Chunk(
      module.OpVectorShuffle(vec4_type, value.id, value.id, components[0], components[1], components[2], components[3]),
      eChunkType::Vector);
  }
  ...
  Sirit::Id ShaderCodeWriterSirit::GetSampledImageType(instr_dimension_t dim) {
    ...
    switch (dim) {
      ...
      case DIMENSION_CUBE:
        ...
        break;
      default: LOG_ERROR(Xenos, "[AST::Emitter] Unsupported texture sample '{}'!", static_cast<u32>(dim)); return {};
    }
  ...
  Chunk ShaderCodeWriterSirit::VectorFunc1(instr_vector_opc_t instr, const Chunk& a) {
    ...
    switch (instr) {
      ...
      case MOVAv:
        ...
        return Chunk(module.OpFloor(vec4_type, a.id),
                     eChunkType::Vector); // Often MOVA means integer floor for address computation
      default: LOG_ERROR(Xenos, "[AST::Emitter] Unsupported vector unary op!"); return Chunk({0}, eChunkType::Vector);
    }
  }
  ...
  Chunk ShaderCodeWriterSirit::VectorFunc3(instr_vector_opc_t instr, const Chunk& a, const Chunk& b, const Chunk& c) {
    ...
    switch (instr) {
      case DOT2ADDv:
        ...
        return Chunk(module.OpFAdd(float_type, module.OpDot(float_type, a.id, b.id), c.id),
                     eChunkType::Vector); // Compute the dot product
      case CUBEv: {
      ...
      case CNDEv:
        ...
        return Chunk(module.OpSelect(vec4_type,
                                     module.OpFOrdGreaterThanEqual(vec4_type, a.id, module.Constant(float_type, 0.5f)),
                                     b.id, c.id),
                     eChunkType::Vector);
      case CNDGTEv:
  ...
  Chunk ShaderCodeWriterSirit::ScalarFunc1(instr_scalar_opc_t instr, const Chunk& a) {
    ...
    // Previous scalar-pipe result (PS) for the *_PREV ops, 0 until a scalar op runs.
    Sirit::Id prev = hasPreviousScalar ? previousScalar : module.Constant(float_type, 0.f);

    Chunk result = Chunk({0}, eChunkType::Scalar);
    switch (instr) {
      case ADDs:
      ...
      case MULs:
        ...
        result = Chunk(module.OpFMul(type, a.id, a.id), eChunkType::Scalar); // a * a
        break;
      case MUL_PREVs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(MUL_PREVs)");
        result = Chunk(module.OpFMul(float_type, a.id, prev), eChunkType::Scalar); // a * ps
        break;
      case MUL_PREV2s:
        // Real HW gates the result on the sign/NaN of ps; approximate as a * ps.
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(MUL_PREV2s)");
        result = Chunk(module.OpFMul(float_type, a.id, prev), eChunkType::Scalar);
        break;
      case SUB_PREVs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(SUB_PREVs)");
        result = Chunk(module.OpFSub(float_type, a.id, prev), eChunkType::Scalar); // a - ps
        break;
      case MAXs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(MAXs)");
        result = Chunk(module.OpFMax(type, a.id, a.id), eChunkType::Scalar); // max(a, a)
        break;
      case MINs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(MINs)");
        result = Chunk(module.OpFMin(type, a.id, a.id), eChunkType::Scalar); // min(a, a)
        break;
      case FRACs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(FRACs)");
        result = Chunk(module.OpFMod(type, a.id, module.Constant(float_type, 1.f)), eChunkType::Scalar); // frac(a)
        break;
      case TRUNCs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(TRUNCs)");
        result = Chunk(module.OpTrunc(type, a.id), eChunkType::Scalar); // trunc(a)
        break;
      case FLOORs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(FLOORs)");
        result = Chunk(module.OpFloor(type, a.id), eChunkType::Scalar); // floor(a)
        break;
      case EXP_IEEE:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc1(EXP_IEEE)");
        result = Chunk(module.OpExp2(type, a.id), eChunkType::Scalar); // 2^x
        break;
      case LOG_CLAMP: {
        ...
        result = Chunk(module.OpLog2(type, clamped)); // log2(max(abs(x), 1e-8f))
        break;
      }
      case LOG_IEEE:
      ...
      default:
        ...
        result = Chunk({0}, eChunkType::Scalar);
        break;
    }

    // Publish this op's result as the next PS.
    previousScalar = result.id;
  ...
  Chunk ShaderCodeWriterSirit::ScalarFunc2(instr_scalar_opc_t instr, const Chunk& a, const Chunk& b) {
    Sirit::Id float_type = module.TypeFloat(32);
    Sirit::Id bool_type = module.TypeBool();
    // 1.0 / 0.0 select operands for the SET* comparisons (which yield 1.0f / 0.0f).
    Sirit::Id one = module.Constant(float_type, 1.f);
    Sirit::Id zero = module.Constant(float_type, 0.f);
    auto boolToFloat = [&](Sirit::Id cond) { return module.OpSelect(float_type, cond, one, zero); };

    Chunk result = Chunk({0}, eChunkType::Scalar);
    switch (instr) {
      case ADDs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc2(ADDs)");
        result = Chunk(module.OpFAdd(float_type, a.id, b.id), eChunkType::Scalar); // a + b
        break;
      case SUBs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc2(SUBs)");
        result = Chunk(module.OpFSub(float_type, a.id, b.id), eChunkType::Scalar); // a - b
        break;
      case MULs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc2(MULs)");
        result = Chunk(module.OpFMul(float_type, a.id, b.id), eChunkType::Scalar); // a * b
        break;
      case MAXs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc2(MAXs)");
        result = Chunk(module.OpFMax(float_type, a.id, b.id), eChunkType::Scalar); // max(a, b)
        break;
      case MINs:
        LOG_DEBUG(Xenos, "[AST::Sirit] ScalarFunc2(MINs)");
        result = Chunk(module.OpFMin(float_type, a.id, b.id), eChunkType::Scalar); // min(a, b)
        break;
      case SETEs:
        result = Chunk(boolToFloat(module.OpFOrdEqual(bool_type, a.id, b.id)), eChunkType::Scalar); // a == b
        break;
      case SETNEs:
        result = Chunk(boolToFloat(module.OpFUnordNotEqual(bool_type, a.id, b.id)), eChunkType::Scalar); // a != b
        break;
      case SETGTs:
        result = Chunk(boolToFloat(module.OpFOrdGreaterThan(bool_type, a.id, b.id)), eChunkType::Scalar); // a > b
        break;
      case SETGTEs:
  ...
  void ShaderCodeWriterSirit::BeginControlFlow(const u32 address, const bool hasJumps, const bool hasCalls,
                                               const bool called) {
    ...
    if (!entry_dispatch_func.value) { entry_dispatch_func = current_function; }
  ...
  void ShaderCodeWriterSirit::PopPredicate() { LOG_DEBUG(Xenos, "[AST::Sirit] PopPredicate()"); }
  ...
  void ShaderCodeWriterSirit::Emit(const Chunk& src) { LOG_DEBUG(Xenos, "[AST::Sirit] Emit({})", src.id.value); }

  void ShaderCodeWriterSirit::AssignMasked(const Chunk& src, const Chunk& dst, std::span<const eSwizzle> dstSwizzle,
    ...
    for (u8 i = 0; i != 4; ++i) { components[i] = module.OpCompositeExtract(float_type, dst.id, i); }
  ...
  void ShaderCodeWriterSirit::AssignImmediate(const Chunk& dst, std::span<const eSwizzle> dstSwizzle,
    ...
    for (u64 i = 0; i != dstSwizzle.size(); ++i) {
      ...
      switch (immediateValues[i]) {
        ...
        default: LOG_ERROR(Xenos, "[AST::Sirit] Invalid immediate swizzle"); continue;
      }
