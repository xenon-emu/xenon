...
namespace Xe::Microcode::AST {
  ...
  public:
    ...
    virtual void AssignMasked(const Chunk& src, const Chunk& dst, std::span<const eSwizzle> dstSwizzle,
                              std::span<const eSwizzle> srcSwizzle) = 0;

    virtual void AssignImmediate(const Chunk& dst, std::span<const eSwizzle> dstSwizzle,
  ...
  public:
    ...
    void AssignMasked(const Chunk& src, const Chunk& dst, std::span<const eSwizzle> dstSwizzle,
                      std::span<const eSwizzle> srcSwizzle) override;

    void AssignImmediate(const Chunk& dst, std::span<const eSwizzle> dstSwizzle,
    ...
    // Previous scalar-pipe result (PS) for the *_PREV scalar ALU ops. Updated by the
    // scalar emitters, hasPreviousScalar stays false until the first scalar op runs.
    Sirit::Id previousScalar = {0};
    bool hasPreviousScalar = false;
