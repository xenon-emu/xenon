...
Block::Block( u32 address, Statement* preamble, Statement* code, ExprNode* condition )
	: m_address( address )
	, m_targetAddrses( 0 )
	, m_target( nullptr )
...
Block::Block( ExprNode* condition, u32 targetAddress, EBlockType type )
	: m_address( 0 )
...
ControlFlowGraph* ControlFlowGraph::DecompileMicroCode( const void* code, const u32 codeLength, XenonShaderType shaderType )
{
	...
	transformer.TransformShader( blockTranslator, (const u32*)code, codeLength / sizeof(u32) );
	...
	std::map< u32, Block* > blocksWithAddresses;
	for ( u32 i=0; i<numBlocks; ++i )
	{
		Block* block = blockTranslator.GetCreatedBlock(i);
		if ( block->GetType() == Block::EBlockType::EXEC )
		{
			...
			{
				printf( "Microcode: Two blocks with the same microcode address %d", block->GetAddress() );
				return nullptr;
			}
	...
	for ( u32 i=0; i<numBlocks; ++i )
	{
		...
		if ( block->GetType() == Block::EBlockType::JUMP )
		{
			...
			{
				printf( "Microcode: No target block with address %d for jump.", block->GetTargetAddress() );
				return nullptr;
			}
		...
		else if ( block->GetType() == Block::EBlockType::CALL )
		{
			...
			if ( targetBlock == nullptr )
			{
				printf( "Microcode: No target block with address %d for call.", block->GetTargetAddress() );
				return nullptr;
			}
...
u32 ControlFlowGraph::GetEntryPointAddress() const
{
...
void ControlFlowGraph::EmitHLSL( class IHLSLWriter& writer ) const
{
	...
	for ( Block* rootBlock : m_rootBlocks )
	{
		...
		const u32 rootAddress = rootBlock->GetAddress();
		...
		for ( u32 j=0; j<allAddressableBlocks.size(); ++j )
		{
	...
	public:
		virtual void OnExprStart( const ExprNode* n ) override final
		{
			...
			else
			{
				...
				if ( regIndex != -1 )
				{
					m_usedRegisters.insert( (u32) regIndex );
				}
			}
		}
		...
		std::set< u32 >							m_usedRegisters;
	};

}

Shader* Shader::DecompileMicroCode( const void* code, const u32 codeLength, XenonShaderType shaderType )
{
	...
	Helper::AllExpressionVisitor visitor(&instructionExtractor);
	Helper::VisitAll( cf, visitor );
	...
	for ( const u32 regIndex : instructionExtractor.m_usedRegisters )
	...
	for ( const auto& exp : ret->m_exports )
	{
		...
		if ( index >= 0 )
		{
			ret->m_numUsedInterpolators = std::max( (u32)index+1, ret->m_numUsedInterpolators );
		}
	}
