...
namespace Helper
{
	...
	public:
		...
		virtual CodeChunk GetReg(u32 regIndex) override final
		{
		...
		virtual CodeChunk GetBoolVal(const u32 boolRegIndex) override final
		{
		...
		virtual CodeChunk GetFloatVal(const u32 floatRegIndex) override final
		{
		...
		virtual CodeChunk GetFloatValRelative(const u32 floatRegOffset) override final
		{
		...
		virtual void BeginControlFlow(const u32 address, const bool bHasJumps, const bool bHasCalls, const bool bIsCalled) override final
		{
		...
		virtual void BeginBlockWithAddress(const u32 address) override final
		{
			if (m_hasJumps)
			{
		...
		virtual void ControlFlowReturn(const u32 targetAddress) override final
		{
		...
		virtual void ControlFlowCall(const u32 targetAddress) override final
		{
			if (m_hasJumps)
			{
		...
		virtual void ControlFlowJump(const u32 targetAddress) override final
		{
...
void CDX11ShaderHLSLGenerator::GenerateRegsStructure(class CDX11ShaderHLSLWriter& writer, const class CDX11MicrocodeShader& microcode)
{
	...
	std::vector< u32 > usedRegs;
	microcode.GetUsedRegisters(usedRegs);
	for (u32 i = 0; i<usedRegs.size(); ++i)
	{
		const u32 regIndex = usedRegs[i];
...
void CDX11ShaderHLSLGenerator::GeneratePixelShaderStructures(class CDX11ShaderHLSLWriter& writer, const Context& context, const class CDX11MicrocodeShader& microcode)
{
	...
	// emit input structure, TODO: we should have rules about generating this
	{
		...
		for (u32 i = 0; i<context.m_numIncomingInputs; ++i)
		{
