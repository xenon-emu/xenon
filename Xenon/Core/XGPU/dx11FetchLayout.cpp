...
CDX11FetchLayout* CDX11FetchLayout::ExtractFromMicrocode(CDX11MicrocodeShader* sourceMicrocode)
{
	...
	// output fetch list
	std::unique_ptr< CDX11FetchLayout > ret(new CDX11FetchLayout());
	...
	for (const auto& vfetch : vertexFetches)
	{
		...
		for (u32 i = 0; i < ret->m_numStreams; ++i)
		{
		...
		if (!stream)
		{
			...
			{
				printf("D3D: Fetch layout has more than %d streams. WTF?", MAX_STREAMS);
				return nullptr;
			}
		...
		if (vfetch.m_stride)
		{
			...
			{
				printf("D3D: Incompatible strides in vertex format %d != %d", vfetch.m_stride, stream->m_stride);
				return nullptr;
			}
		}
