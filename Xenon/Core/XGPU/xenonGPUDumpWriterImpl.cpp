...
void CXenonGPUDumpWriterImpl::Close()
{
	...
	while ( !m_blockStack.empty() )
	{
		...
		block.m_numPackets = (u32) m_tables.m_packets.size() - block.m_firstPacket;
		block.m_numSubBlocks = (u32) m_tables.m_blocks.size() - block.m_firstSubBlock;
...
void CXenonGPUDumpWriterImpl::BeginBlock(const char* tag, u32& outBlockID )
{
	CXenonGPUDumpFormat::Block block;
	block.m_firstPacket = (u32) m_tables.m_packets.size();
	block.m_firstSubBlock = (u32) m_tables.m_blocks.size() + 1;
	block.m_numSubBlocks = 0;
	block.m_numPackets = 0;

	strncpy_s( block.m_tag, tag, ARRAYSIZE(block.m_tag) - 1 );

	const u32 blockId = (u32) m_tables.m_blocks.size();
	m_tables.m_blocks.push_back( block );

	m_blockStack.push_back( blockId );
	printf( "GPU TRACE: Block %d started (%hs)\n", blockId, tag );

	outBlockID = blockId;
}

void CXenonGPUDumpWriterImpl::EndBlock( u32 blockId )
{
	...
	printf( "GPU TRACE: Block %d finished, %d sub blocks, %d packets\n", blockId, block.m_numSubBlocks, block.m_numPackets );
}

void CXenonGPUDumpWriterImpl::Packet( const u32 packetType, const void* dataWords, const unsigned int lengthInWords )
{
	// add packet info
	...
	packet.m_firstDataWord = (u32) m_tables.m_dataRegs.size();
	for ( u32 i=0; i<lengthInWords; ++i )
		m_tables.m_dataRegs.push_back( _byteswap_ulong( ((const u32*) dataWords)[i]) );

	// setup memory references
	packet.m_numMemoryRefs = 0;
	packet.m_firstMemoryRef = (u32) m_tables.m_memoryRefs.size();

	// add to packet list
	m_tables.m_packets.push_back( packet );
	printf( "GPU TRACE: Packet 0x%08X, %d words\n", packetType, lengthInWords );
}

void CXenonGPUDumpWriterImpl::MemoryAccessRead( const u64 realAddress, const u32 size, const char* context )
{
	// empty block
	if ( !realAddress || !size )
		return;
	...
	// map memory block
	const u32 blockIndex = MapMemoryBlock( realAddress, size );
	...
	// add to packet
	const u32 refIndex = (u32) m_tables.m_memoryRefs.size() - 1;
...
void CXenonGPUDumpWriterImpl::MemoryAccessWrite( const u64 realAddress, const u32 size, const char* context )
{
	// empty block
	if ( !realAddress || !size )
		return;
...
namespace Helper
{
	...
	u64 Crc64( u64 crc, const unsigned char *s, u64 l)
	{
		for (u64 j = 0; j < l; j++)
		{
			uint8_t byte = s[j];
...
u32 CXenonGPUDumpWriterImpl::MapMemoryBlock( const u64 address, const u32 size )
{
	// get true data pointer
	const void* ptr = (const void*) address;

	// compute the data CRC
	const u64 crc = Helper::Crc64( 0, (const unsigned char*) ptr, size );

	// make the search key :)
	auto keyHigh = ((u64)address << 32) | size;
	...
	// add to map
	const u32 blockIndex = (u32) m_tables.m_memoryBlocks.size() - 1;
	m_memoryMap[key] = blockIndex;

	// stats
	printf( "GPU TRACE: New memory block %d: 0x%08X, size %d, CRC: 0x%016llX\n", 
