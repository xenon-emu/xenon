...
bool CDX11AbstractLayer::Initialize()
{
	...
	m_drawer = new CDX11GeometryDrawer( m_device, m_mainContext, ram );
	...
	if (m_eventDevice)
	{
		...
		{
			printf("DX11: Profiling event tracking enabled");
		}
	}

	// done
	printf( "DX11: Abstract layer initialized" );
	return true;
}

bool CDX11AbstractLayer::SetDisplayMode( const u32 width, const u32 height )
{
	m_canRender = false;

	printf( "DX11: Requesting mode change to %dx%d", width,height );
	return true;
}
...
void CDX11AbstractLayer::Swap( const CXenonGPUState::SwapState& ss )
{
	...
	if (0) //swapTexture )
	{
		...
		const u32 maxWidth = frontBufferDesc.Width;
		const u32 maxHeight = frontBufferDesc.Height;

		// calculate the amount of data to copy
		const u32 copyMinX = 0;
		const u32 copyMinY = 0;
		const u32 copyMaxX = maxWidth;
		const u32 copyMaxY = maxHeight;
...
bool CDX11AbstractLayer::CreateDevice()
{
	...
	{
		printf( "DX11: Unable to create device, err = 0x%08X", hRet );
		return false;
	}
...
//#endif
	...
	// device initialized
	printf( "DX11: Native device initialized" );
	return true;
}
...
void CDX11AbstractLayer::DiscardSwapchain()
{
	...
	// stats
	printf( "DX11: Swapchain discarded" );
}
...
bool CDX11AbstractLayer::CreateSwapchain()
{
	...
	if ( FAILED( hr ) )
	{
		printf( "DXGI: Failed to get adapter GIDevice, hRet: 0x%0X", (u32)hr );
		return false;
	}
	...
	if ( FAILED( hr ) )
	{
		printf( "DXGI: Failed to get adapter GIAdapter, hRet: 0x%0X", (u32)hr );
		return false;
	}
	...
	if ( FAILED( hr ) )
	{
		printf( "DXGI: Failed to get adapter GIFactory, hRet: 0x%0X", (u32)hr );
		return false;
	}
	...
	if ( FAILED( hr ) )
	{
		printf( "DXGI: Failed to create swap chain, hRet: 0x%0X", (u32)hr );
		return false;
	}
	...
	{
		swapChain->Release();
		printf( "DXGI: Failed to get back buffer surface, hRet: 0x%0X", (u32)hr );
		return false;
	}
	...
	if ( FAILED( hr ) || backBufferView == NULL )
	{
		swapChain->Release();
		backBuffer->Release();
		printf( "DXGI: Failed to get back buffer view, hRet: 0x%0X", (u32)hr );
		return false;
	}
	...
	// Swapchain created
	printf( "DX11: Swapchain created, mode: %dx%d, fullscreen: %d", scDesc.BufferDesc.Width, scDesc.BufferDesc.Height, !scDesc.Windowed );
	return true;
}

void CDX11AbstractLayer::BindColorRenderTarget( const u32 index, const XenonColorRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch )
{
...
void CDX11AbstractLayer::BindDepthStencil( const XenonDepthRenderTargetFormat format, const XenonMsaaSamples msaa, const u32 base, const u32 pitch )
{
	// HACK: we don't support properly the MSAA, force the x1 mode but save what it should be for other hacks (mainly clear)
	m_depthMSAA = msaa;
...
void CDX11AbstractLayer::UnbindColorRenderTarget( const u32 index )
{
...
void CDX11AbstractLayer::SetColorRenderTargetWriteMask( const u32 index, const bool enableRed, const bool enableGreen, const bool enableBlue, const bool enableAlpha )
{
	DEBUG_CHECK( index < 4 );
...
void CDX11AbstractLayer::ClearColorRenderTarget( const u32 index, const float* clearColor, const bool flushToEDRAM )
{
	m_surfaceManager->ClearColor( index, clearColor, flushToEDRAM );
}

void CDX11AbstractLayer::ClearDepthStencilRenderTarget( const float depthClear, const u32 stencilClear, const bool flushToEDRAM )
{
	m_surfaceManager->ClearDepth( depthClear, stencilClear, flushToEDRAM );
}

bool CDX11AbstractLayer::RealizeSurfaceSetup( u32& outMainWidth,  u32& outMainHeight )
{
...
bool CDX11AbstractLayer::ResolveColorRenderTarget( const u32 srcIndex, const XenonColorRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, const u32 destBase, const u32 destLogicalWidth, const u32 destLogicalHeight, const u32 destBlockWidth, const u32 destBlockHeight, const XenonTextureFormat destFormat, const XenonRect2D& destRect )
{
...
bool CDX11AbstractLayer::ResolveDepthRenderTarget( const XenonDepthRenderTargetFormat srcFormat, const u32 srcBase, const XenonRect2D& srcRect, const u32 destBase, const u32 destLogicalWidth, const u32 destLogicalHeight, const u32 destBlockWidth, const u32 destBlockHeight, const XenonTextureFormat destFormat, const XenonRect2D& destRect )
{
...
void CDX11AbstractLayer::EnableScisor( const u32 x, const u32 y, const u32 w, const u32 h )
{
...
void CDX11AbstractLayer::SetStencilWriteMask( const u8 mask )
{
...
void CDX11AbstractLayer::SetStencilReadMask( const u8 mask )
{
	const UINT8 value = mask;
...
void CDX11AbstractLayer::SetStencilRef( const u8 mask )
{
...
void CDX11AbstractLayer::SetBlend( const u32 rtIndex, const bool isEnabled )
{
...
void CDX11AbstractLayer::SetBlendOp( const u32 rtIndex, const XenonBlendOp colorOp, const XenonBlendOp alphaOp )
{
	DEBUG_CHECK( rtIndex < 4 );
...
void CDX11AbstractLayer::SetBlendArg( const u32 rtIndex, const XenonBlendArg colorSrc, const XenonBlendArg colorDest, const XenonBlendArg alphaSrc, const XenonBlendArg alphaDest )
{
	DEBUG_CHECK( rtIndex < 4 );
...
void CDX11AbstractLayer::SetPrimitiveRestartIndex( const u32 index )
{
...
bool CDX11AbstractLayer::RealizeRasterState()
{
	...
	{
		printf( "D3D: Primitive restart is enabled with index=0x%04X. This is not supported.", m_primitiveRestartIndex );
		return false;
	}
...
void CDX11AbstractLayer::SetPixelShader( const void* microcode, const u32 numWords )
{
	...
	if (microcode)
		m_vertexShaderHash = XenonGPUCalcCRC(microcode, numWords * sizeof(u32));
	else
		m_vertexShaderHash = 0;

}

void CDX11AbstractLayer::SetVertexShader( const void* microcode, const u32 numWords )
{
	m_drawer->SetVertexShaderdCode( microcode, numWords );

	if (microcode)
		m_pixelShaderHash = XenonGPUCalcCRC(microcode, numWords * sizeof(u32));
	else
		m_pixelShaderHash = 0;
}

void CDX11AbstractLayer::SetPixelShaderConsts( const u32 firstVector, const u32 numVectors, const float* values )
{
	float* dest = &m_pixelShaderConsts.Get().m_values[ firstVector*4 ];
	for ( u32 i=0; i<numVectors; ++i, dest += 4, values += 4 )
	{
		dest[0] = values[0];
		dest[1] = values[1];
		dest[2] = values[2];
...
void CDX11AbstractLayer::SetVertexShaderConsts( const u32 firstVector, const u32 numVectors, const float* values )
{
...
void CDX11AbstractLayer::SetBooleanConstants( const u32* boolConstants )
{
...
bool CDX11AbstractLayer::HACK_ClearFromDraw(const CXenonGPURegisters& regs, IXenonGPUDumpWriter* traceDump, const CXenonGPUState::DrawIndexState& ds, RAM* ram)
{
	DEBUG_CHECK(ds.m_indexCount == 3);

	// get the vertex stream with clear color
	const u32 fetchSlot = 0;
	const u32 fetchRegBase = (u32)XenonGPURegister::REG_SHADER_CONSTANT_FETCH_00_0 + (fetchSlot * 2);
	...
	const u32 srcMemoryAddress = ((fetchEntry.address << 2) + 0);

	// get the area to clear - 3 corners
	float rectArea[3][3];
	u32 data = 0;
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 0 + 0), sizeof(float));
	rectArea[0][0] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 0 + 4), sizeof(float));
	rectArea[0][1] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 0 + 8), sizeof(float));
	rectArea[0][2] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 28 + 4), sizeof(float));
	rectArea[1][0] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 28 + 8), sizeof(float));
	rectArea[1][1] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 28 + 4), sizeof(float));
	rectArea[1][2] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 56 + 8), sizeof(float));
	rectArea[2][0] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 56 + 4), sizeof(float));
	rectArea[2][1] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 56 + 8), sizeof(float));
	rectArea[2][2] = std::bit_cast<float>(byteswap_be(data));
	/*
	...
	*/
	...
	//if (m_colorMSAA[0] == XenonMsaaSamples::MSSA4X)
	{
		...
		data = byteswap_be(std::bit_cast<u32>(rectArea[0][0]));
		memcpy( ram->GetPointerToAddress(srcMemoryAddress + 0 + 0), &data, sizeof(u32));
		data = byteswap_be(std::bit_cast<u32>(rectArea[0][1]));
		memcpy(ram->GetPointerToAddress(srcMemoryAddress + 0 + 4), &data, sizeof(u32));

		data = byteswap_be(std::bit_cast<u32>(rectArea[1][0]));
		memcpy(ram->GetPointerToAddress(srcMemoryAddress + 28 + 0), &data, sizeof(u32));
		data = byteswap_be(std::bit_cast<u32>(rectArea[1][1]));
		memcpy(ram->GetPointerToAddress(srcMemoryAddress + 28 + 4), &data, sizeof(u32));

		data = byteswap_be(std::bit_cast<u32>(rectArea[2][0]));
		memcpy(ram->GetPointerToAddress(srcMemoryAddress + 56 + 0), &data, sizeof(u32));
		data = byteswap_be(std::bit_cast<u32>(rectArea[2][1]));
		memcpy(ram->GetPointerToAddress(srcMemoryAddress + 56 + 4), &data, sizeof(u32));
		/*
	...
	float clearColor[4];
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 12 + 0), sizeof(float));
	clearColor[0] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 12 + 4), sizeof(float));
	clearColor[1] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 12 + 8), sizeof(float));
	clearColor[2] = std::bit_cast<float>(byteswap_be(data));
	memcpy(&data, ram->GetPointerToAddress(srcMemoryAddress + 12 + 12), sizeof(float));
	clearColor[3] = std::bit_cast<float>(byteswap_be(data));
	/*
	...
	*/
	...
	// update colors
	data = byteswap_be(std::bit_cast<u32>(clearColor[0]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 12 + 0), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[1]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 12 + 4), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[2]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 12 + 8), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[3]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 12 + 12), &data, sizeof(u32));

	data = byteswap_be(std::bit_cast<u32>(clearColor[0]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 28 + 12 + 0), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[1]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 28 + 12 + 4), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[2]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 28 + 12 + 8), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[3]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 28 + 12 + 12), &data, sizeof(u32));

	data = byteswap_be(std::bit_cast<u32>(clearColor[0]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 56 + 12 + 0), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[1]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 56 + 12 + 4), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[2]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 56 + 12 + 8), &data, sizeof(u32));
	data = byteswap_be(std::bit_cast<u32>(clearColor[3]));
	memcpy(ram->GetPointerToAddress(srcMemoryAddress + 56 + 12 + 12), &data, sizeof(u32));
	/*
...
bool CDX11AbstractLayer::DrawGeometry( const CXenonGPURegisters& regs, IXenonGPUDumpWriter* traceDump, const CXenonGPUState::DrawIndexState& ds, RAM *ram)
{
	// clear shader - HACK
	if ( ds.m_primitiveType == XenonPrimitiveType::PrimitiveRectangleList && ds.m_indexCount == 3 )
	{
		if (HACK_ClearFromDraw(regs, traceDump, ds, ram))
			return true;
	}

	// draw
	XenonGPUScope scope(this, "Draw, PT=%hs, IndexCount=%d", XenonGetPrimitiveTypeName(ds.m_primitiveType), ds.m_indexCount);
	return m_drawer->Draw( regs, traceDump, ds, ram);
}

//---------------------------------------------------------------------------

u32 CDX11AbstractLayer::GetActiveTextureFetchSlotMask() const
{
	return m_drawer->GetActiveTextureFetchSlotMask();
}

void CDX11AbstractLayer::SetTexture( const u32 fetchSlot, const XenonTextureInfo* texture )
{
	...
	if ( texture && texture->m_address )
	{
		...
		if ( !runtimeTexture )
			printf( "D3D: Failed to build runtime texture" );
	}
...
void CDX11AbstractLayer::SetSampler(const u32 fetchSlot, const XenonSamplerInfo* sampler)
{
