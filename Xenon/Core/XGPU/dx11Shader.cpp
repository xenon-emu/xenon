...
u32 CDX11VertexShader::DrawingContext::GetHash() const
{
	u32 ret = 0;
	ret |= (m_bTest ? 1 : 0) << 0;
	ret |= (m_psRegs & 0x3F) << 1;
...
CDX11VertexShader* CDX11VertexShader::Compile( ID3D11Device* device, CDX11MicrocodeShader* sourceMicrocode, const DrawingContext& context, const std::wstring& dumpDir )
{
	...
	{
		printf( "D3D: Cannot build vertex shader from microcode for pixel shader" );
		return nullptr;
	}
	...
	{
		printf( "D3D: Error converting microcode shader hash 0x%016llX to HLSL", sourceMicrocode->GetHash() );
		return nullptr;
	}
	...
	if ( !fetchLayout )
	{
		printf( "D3D: Error extracting vertex layout from microcode shader hash 0x%016llX", sourceMicrocode->GetHash() );
		return nullptr;
	}
	...
	if ( FAILED( hRet ) )
	{
		...
		printf( "D3D: Failed to compile vertex shader: %hs", errorText );
	...
	if ( FAILED(hRet) )
	{
		printf( "D3D: Failed to create vertex shader, error 0x%08X", hRet );
		codeBlob->Release();
		return nullptr;
	}
	...
	{
		printf( "D3D: Failed to create vertex input layout, error 0x%08X", hRet );
		codeBlob->Release();
		vs->Release();
		return nullptr;
	}
...
u32 CDX11PixelShader::DrawingContext::GetHash() const
{
	u32 ret = 0;
...
CDX11PixelShader* CDX11PixelShader::Compile( ID3D11Device* device, CDX11MicrocodeShader* sourceMicrocode, const DrawingContext& context, const std::wstring& dumpDir )
{
	...
	if ( !sourceMicrocode->IsPixelShader() )
	{
		printf( "D3D: Cannot build pixel shader from microcode for vertex shader" );
		return nullptr;
	}
	...
	if ( FAILED( hRet ) )
	{
		// report errors
		const char* errorText = errorBlob ? (const char*) errorBlob->GetBufferPointer() : "";
		printf( "D3D: Failed to compile pixel shader: %hs", errorText );
	...
	if ( FAILED(hRet) )
	{
		printf( "D3D: Failed to create pixel shader, error 0x%08X", hRet );
		codeBlob->Release();
		return nullptr;
	}
