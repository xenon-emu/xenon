...
ExprNode::~ExprNode()
{
	...
	for ( int32_t i=0; i<ARRAYSIZE(m_children); ++i )
	{
		if ( m_children[i] )
...
std::string ExprReadSwizzle::ToString() const
{
	...
	ret += swizzles[ (u8)m_swizzles[0] ];
	ret += swizzles[ (u8)m_swizzles[1] ];
	ret += swizzles[ (u8)m_swizzles[2] ];
	ret += swizzles[ (u8)m_swizzles[3] ];
...
CodeChunk ExprReadSwizzle::EmitHLSL( IHLSLWriter& writer ) const
{
	...
	const char* swizzles[] = {"x", "y", "z", "w"};
	ret.Append( swizzles[ (u8)m_swizzles[0] ] );
	ret.Append( swizzles[ (u8)m_swizzles[1] ] );
	ret.Append( swizzles[ (u8)m_swizzles[2] ] );
	ret.Append( swizzles[ (u8)m_swizzles[3] ] );
	ret.Append( ")" );
...
ExprVertexFetch::ExprVertexFetch( ExprNode* src, u32 fetchSlot, u32 fetchOffset, u32 fetchStride, EFetchFormat format, const bool isFloat, const bool isSigned, const bool isNormalized )
	: ExprNode( src )
	, m_format( format )
...
ExprTextureFetch::ExprTextureFetch( ExprNode* src, u32 fetchSlot, TextureType textureType )
	: ExprNode( src )
	, m_fetchSlot( fetchSlot )
...
void WriteWithMaskStatement::EmitHLSL( IHLSLWriter& writer ) const
{
	...
	ESwizzle destMask[ 4 ];
	u32 numSourceSwizzles = 0;
	u32 numSpecialChannels = 0;
	for ( u32 i=0; i<ARRAYSIZE(m_mask); ++i )
	{
	...
	if ( numSourceSwizzles > 0 )
	{
		...
		swizzledDest.Append( "." );
		for ( u32 i=0; i<numSourceSwizzles; ++i )
		{
			const char* masks[] = { "x", "y", "z", "w" };
			DEBUG_CHECK( (u32)sourceSwizzles[i] < ARRAYSIZE(masks) );
			swizzledDest.Appendf( masks[ (u32)destMask[i] ] );

			// HLSL compiler bug workaround: use the same swizzles on source
			swizzledSrc.Appendf( masks[ (u32)sourceSwizzles[i] ] );
		}
