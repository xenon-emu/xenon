...
void OHCI::ProcessControlList() {
  if (HcControlHeadED == 0 || !transferProcessor) {
    return;
  }
...
void OHCI::ProcessPeriodicList() {
  if (HcHCCA == 0 || !transferProcessor || !ramPtr) {
    return;
  }
...
void OHCI::SetInterrupt(u32 interrupt) {
  ...
  if ((HcInterruptEnable & interrupt) &&
    ...
    // TODO: Trigger actual interrupt to CPU
    LOG_DEBUG(OHCI, "{} Interrupt triggered: 0x{:X}", instance, interrupt);
  }
}

void OHCI::MemoryRead(u32 address, u8* data, u32 size) {
  if (ramPtr && address != 0) {
    ramPtr->Read(address, data, size);
  }
}

void OHCI::MemoryWrite(u32 address, const u8* data, u32 size) {
  if (ramPtr && address != 0) {
    ramPtr->Write(address, data, size);
  }
}
...
break;
  ...
  default:
    if (offset >= 0x54 && offset < 0x54 + (ports * 4)) {
      ...
      LOG_DEBUG(OHCI, "{} HcRhPortStatus[{}] == 0x{:08X}", instance, portIndex, ret);
    }
    break;
  }
